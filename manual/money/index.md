...
|---|---|---|---|---|
| What a balance is | fUSD (fake dollars) recorded by the operator | a test token on Arbitrum Sepolia | USDT0 on Arbitrum One | USDT0 on Polygon |
| What it is worth | nothing | nothing | dollars | dollars |
| Register a paying address | refused: this network has no addresses | required, by signing a message | required, by signing a message | required, by signing a message |
| How you deposit | ask the operator; there is nothing to send | send the token to the kernel's address | send USDT0 to the kernel's address | send USDT0 to the kernel's address |
| When you are credited | when the operator records it | after block inclusion and detection by the kernel | after finality and detection by the kernel | after finality and detection by the kernel |
