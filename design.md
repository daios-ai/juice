...
Status: the complete, lossless classification of `requirements.md`, which stays authoritative until rewritten against this document. The user stories below are the intended basis of the Juice standard.
...
- §10 is a lag ledger: a decision folds into the body as current contract, and §10 records only what still diverges in `requirements.md` or the code, each line deleted as its lag closes. An empty ledger means document, contract, and system agree.
...
- **U43** First boot yields a working, network-joined kernel after supplying only a password and a kernel name; safe defaults for everything else. The operator recovers the superuser password like any user. (Behavior when no name is supplied is contradictory in the spec — open issue 2.)
- **U44** The operator can observe everything material — parked remote calls with age, peers with balances, evidence about any subject, unresolved transfers — and intervenes with the same commands users have, widened in scope, plus the money-and-trust verbs only an operator performs.
...
Normative for interoperability; versioned; today upgraded in lockstep (§13).

### P1 Identity and signatures (§12, §13)
...
### P3 Streams (§13)
Versioned libp2p streams: `/juice/fed/call/1` (inbound call), `/juice/fed/step/1` (step list + completion), `/juice/fed/resolve/1` (open read-only single-action manifest or handle→PrincipalID resolution — the sole cache-fill path), `/juice/fed/gossip/1` (cursored catalog + evidence), `/juice/fed/settle/1` (residual settlement). No inspect protocol — inspect is served by gossip. A payload change keeps its stream id and surfaces as a signature failure; the network upgrades in lockstep. Timestamp windows: request age ≤ 5 min (call, step). [carrier: D12]
...
### P5 Receipt (§3, §11, §13)
Fields: `id, issuer_user_id, tx_id, trace_id, action_id, caller_user_id, process_id, args_hash, reply_hash, status, gross, net, fee, charge, premium, value, value_premium, value_to, reason, started_at, created_at, signature`. `args_hash`/`reply_hash` = SHA-256 over JCS args/reply. `gross` = full allocation; `fee + net` = amount remaining at settlement; `gross − fee − net` = committed downstream; failure has `fee = net = 0`. `charge` = actually drawn: `= gross` success, `≤ gross` failure (settled descendants stay paid), `0` rejection. `premium = ceil(charge·remote_bps/10000)`. `value ∈ {0, amount}` (all-or-nothing); `value_premium = ceil(value·remote_bps/10000)` — always 0, the value channel being local and untaxed; retained so older signed receipts stay verifiable; premiums computed per channel, never folded; JCS omitempty keeps pre-transfer receipts verifying. Signature Ed25519 over canonical JSON minus `signature`. Exactly one receipt per committed call; `started_at` call start, `created_at` settlement. [→U36, G1, G7]
...
### P7 Cross-kernel pricing and settlement (§8, §13)
`sr = mp + ceil(mp·remote_bps/10000)` — the cross-kernel obligation, computed on the **actual** charge; the origin adds locally `q = sr + ceil(sr·import_bps/10000)` (import fee retained, never on the wire). `remote_bps` prices the serving side's credit, default, and settlement variance; the caller can never be charged more than the authenticated `q`. Settlement occurs **only** on a signed remote receipt — never on timeout:
...
- signed rejection: full refund; `refresh_proxy` deactivates the proxy iff the row still holds the dispatched hash (a stale dispatch spares a refreshed row); 402 exposure rejection settles as `ErrPeerUnfunded` (operator condition, no refresh).
...
- `complete` signs `JCS({counterparty, idempotency_key, input_hash, recipient, step_id, timestamp})`; `input_hash` = SHA-256 of the exact bytes carried (sender normalizes input to the transported bytes); key **derived** = `SHA-256(protocol | peer key | step_id | input_hash)` so a retry recovers the stored outcome; different input or step derives a different key. Unknown key refused outright. Executes as the peer's account (role law: caller = kernel account); settles wholly on the serving kernel; the requester parks nothing, creates no trace or transaction — failures are typed errors, not rejection receipts. Reuses the P4 idempotency record; a replay is rebuilt from stored result + receipt and discriminates on the **receipt's** status (a result containing an `error` key still replays as success); a stored failure replays as failure with a typed code. Recipient binding prevents cross-kernel replay of list and complete alike. Outbound distinguishes never-dispatched (`ErrPeerUnreachable`) from possibly-executed (`ErrTimeout`, retry under the derived key).
- No payment descriptor rides the step listing, and a step whose action bears the transfer effect is not completable by a peer: value is local to a kernel. The completion key keeps a reserved trailing component so keys stay stable across that removal.
- `step_auth`: a home-kernel attestation naming `required_caller_remote_id`, demanded when a step is addressed to a remote principal rather than the peer kernel itself — a remote handle rename never mis-addresses a parked step (§3).

### P9 Gossip and evidence (§13)
...
### D4 Domain model and action lifecycle (§3, §7, §12)
...
- **Action**: `id, owner_user_id, name, kind ∈ {http, wasm, native, remote_proxy}, active, visibility, price, effect, description, input_schema, output_schema, source, auth_json, artifact_hash, remote_action_id, remote_owner_id, remote_bps, base_price, created_at, updated_at`. `(owner, name)` unique; `/` allowed in names, not handles, so `owner/name` is unambiguous. Default private, widened only by update. `auth_json` write-only, encrypted; reads expose only `auth_scheme` + `requires_grant` [→G5]. Active requires non-empty description, valid schemas, field descriptions sufficient for lookup and LLM calling. Inactive ⇒ uncallable.
- Lifecycle (§7): create inactive; validate owner/name/kind/price; WASM validates/compiles only with executor configured; HTTP stores one structured source (verb — default POST — base URL, path, bindings) shared by manual and imported rows. Source-URL safety per G9: reject non-HTTP(S), RFC 1918, link-local, unspecified, CGNAT at create and activate; loopback permitted by default; `allow_local_sources` widens; a loopback source redirecting to a private address is still blocked. Activate: owner authority, init stats, reject invalid schema/source/artifact/runtime. Update: owner authority; contract-touching changes deactivate unless safe; recompute artifact hash; preserve historical source/hash. Delete: owner authority, soft, history preserved [→U9]. `RegisterNativeAction` bootstrap-only, `sys`-owned; normal create rejects `kind=native` (D17).
...
- **Transaction**: immutable; `id, process_id, trace_id, parent_trace_id`, the role triple, `action_id, action_name` (captured at creation — history self-contained after deletion), `args_json, reply_json, status ∈ {success, failure}, gross, net, fee, reason, remote_receipt_hash/json, started_at, ended_at`. One per attempted call [→G1, G2]. `reason` is the failure class only — never an upstream URL, body, or SQL, and a peer's reason is never adopted locally [→G5].
...
- **LedgerEntry**: `id, operator_user_id, from_user_id, to_user_id, amount, reason, external_key, created_at`; deposit from-null, withdrawal to-null, transfer both set; at least one side non-null; amount positive; debit requires funds; `external_key` globally-unique opaque idempotency token, never interpreted — kernel stays rail-agnostic [→U3].
- **Rating**: `id, rated_tx_id, rated_receipt_id` (null only pre-receipt), `rated_receipt_hash` (portable evidence link; empty ⇒ local-stats-only, never gossiped), `rater_user_id, rating ∈ {0,1}` [convention], `note ≤ 1024 B` (nullable, inside the single signature), `created_at, signature`. One per transaction.
...
### D10 Upstream and delegated auth (§8)
...
- `sys/llm/chat`: `messages[{role, content}], system?` → `message`; `ErrInvalidState` unconfigured.
- `sys/llm/embed`: `text` → `embedding`; `ErrInvalidInput` empty; `ErrInvalidState` unconfigured.
...
- `sys/llm/decide`: `messages[{role, content?, tool?{action, args, result}}], actions[]` → `{action, args, message?}`; contracts fetched from the DB by ref, args validated; selects, never executes [→U46]; kernel-qualified candidates resolve via D13 — dead ones discarded, `ErrNotFound` only when none resolves; bare unknown refs strict `ErrNotFound`; `ErrInvalidState` no tool calling; `ErrExecutionFailed` no valid selection.
- `sys/time`: → `{unix, iso}`; `sys/random`: → `value ∈ [0,1)`; `sys/sink`: any → `{}` — the explicit clock, randomness, and no-op the deterministic sandbox withholds (random exists because scripts have no OS entropy).
...
- `sys/web`: `url` → `{status, body, content_type, final_url}`; GET only; no caller headers/auth (nothing sensitive enters args/receipts/logs); scheme-less defaults https, explicit scheme never downgraded; SSRF per G9; non-2xx returned, not raised; 10 MiB cap [policy]; UA derived from the binary version; `ErrInvalidInput`/`ErrInvalidState`/`ErrExecutionFailed` typed. The mediated web path for scripts [→G9].
- `sys/tinygo/compile`: `source` → `{status, artifact (b64), artifact_hash, diagnostics}`; price 5 [policy]; SDK prepended so authors write `Handle(in) (out, error)`; author compile/validation errors are charged output-failures; missing toolchain `ErrInvalidState` uncharged; registration separate (`action create --kind wasm --artifact`).

### D18 Value transfer (§13) — local only
...
### D19 Dispatch records and retry (§13)
...
## 10. Deviations from `requirements.md`

The sixteen rulings of 2026-08-14 are executed: `requirements.md`, the code, the tests, and the flows now agree with this document, so the ledger that tracked their lag is empty. Engineering choices made while executing them, recorded because they bound future edits:
...
11. Native encapsulation — constraint (requirements.md §9): natives are ordinary actions behind `Call()`, no kernel privileges; which natives exist design (D17).
...
- `requirements.md` §16 (design rationale) is justification, not contract: its content is preserved as commentary beside the entries it argues for (D2, D5, D12, D14, D15, D22; ruling texts in §11).
- The file-header editing instructions (`requirements.md` lines 7–9) are editorial meta.
- Contract deviations, when any exist, are recorded in §10.

## 14. Coverage

Every section of `requirements.md` is absorbed as follows; nothing else remains:
