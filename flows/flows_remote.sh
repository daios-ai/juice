...
flow_refresh_rotation() {
    ...
    assert_json "refresh_rotation.refresh_succeeds" "$(jj "$db" "$ha" user me)" address alice@k
...
flow_successful_receipt() {
    ...
    local out; out=$(jj "$db" "$hb" run alice@k/receipt-action '{}')
...
flow_failed_receipt() {
    ...
    j "$db" "$hb" run alice@k/fail-action '{}' >/dev/null 2>&1 || true
...
flow_lookup() {
    ...
    assert_fails "lookup.missing_query_rejected" "query\|required\|schema" -- j "$db" "$ha" run sys@k/lookup '{}'
    ...
    local hit; hit=$(jj "$db" "$ha" run sys@k/lookup '{"query":"zqxwvprobe"}')
    assert_eq "lookup.hit_carries_evidence" ok "$(python3 -c "
import sys, json
rows = (json.loads(sys.argv[1]).get('result') or {}).get('results') or []
hit = next((r for r in rows if r.get('action') == 'alice@k/zqxwvprobe'), None)
...
flow_chat() {
    ...
    local out; out=$(j "$db" "$ha" run sys@k/llm/chat '{"messages":[{"role":"user","content":"hello"}]}' 2>&1)
...
PY
    ...
    j "$db" "$ha" action enable alice@k/mail >/dev/null 2>&1
    j "$db" "$ha" action update alice@k/mail --visibility public >/dev/null 2>&1

    # The group answers to its own name: reading and running alice/mail both reach the index.
    assert_json "openapi_application.show_root" "$(jj "$db" "$hb" action show alice@k/mail)" id "$idx_id"
    assert_nonempty "openapi_application.run_root" "$(strfield "$(jj "$db" "$hb" run alice@k/mail '{}')" tx_id)"
    assert_nonempty "openapi_application.run_member" "$(strfield "$(jj "$db" "$hb" run alice@k/mail/greet '{}')" tx_id)"
    # Members keep their own identity; the index does not shadow them.
    assert_json "openapi_application.member_distinct" "$(jj "$db" "$hb" action show alice@k/mail/greet)" id "$greet_id"
...
PY
    ...
    assert_contains "openapi_disable.names_rows" "mail/greet" "$(j "$db" "$ha" action disable alice@k/mail)"
    ...
    j "$db" "$ha" action delete alice@k/mail >/dev/null 2>&1
