...
# A provider exposes two actions under one directory that call an upstream API on the caller's behalf.
# The caller connects the whole directory with ONE pasted token (one connection, two grants), runs
# both — the live upstream confirms each received exactly that token — sees one connection covering
# two actions in `user me` (token-free), then disconnects the whole account and is rejected again.
flow_grant_bearer() {
    echo "=== FLOW grant_bearer ==="
    local dir db hs ha a1 a2 bport
    dir=$(new_dir); db="$dir/juice.db"; hs=$(home "$dir" sys); ha=$(home "$dir" alice)
...
# flow_step_gates — @sys/step/race and @sys/step/join (§9), the two ways a workflow waits on
# several things at once. Both resume an ONWARD step created by the workflow, so the continuation's
# price is parked once rather than once per contributor. Driven through the CLI: @sys/message parks
# a step (holding the process open), and further steps are created on that same root trace — the
# gates only fire within their own process, so the shared trace is what makes them reachable.
flow_step_gates() {
    echo "=== FLOW step_gates ==="
    local dir db hs ha hb hc
    dir=$(new_dir); db="$dir/juice.db"; hs=$(home "$dir" sys); ha=$(home "$dir" alice); hb=$(home "$dir" bob); hc=$(home "$dir" carol)
    start_server "$db" "$hs" || { fail "step_gates.boot" "server did not start"; return; }
    j "$db" "$hs" auth login @sys --password sys-pass >/dev/null 2>&1
    make_user "$db" "$hs" "$ha" @alice
    make_user "$db" "$hs" "$hb" @bob
    make_user "$db" "$hs" "$hc" @carol

    # Open a process and learn its root trace: the anchor every step below hangs off.
    local seed trace
    seed=$(resultf "$(jj "$db" "$ha" run @sys/message '{"to":"@bob","message":"kick off"}')" step_id)
    trace=$(strfield "$(jj "$db" "$ha" step show "$seed")" parent_trace_id)
    assert_nonempty "step_gates.trace" "$trace"

    # --- race: two contributors, one winner ---
    local onward r1 r2
    onward=$(strfield "$(jj "$db" "$ha" step create @sys/sink --trace "$trace" --required-caller @sys --partial-args '{}')" id)
    assert_nonempty "step_gates.race_onward_created" "$onward"
    r1=$(strfield "$(jj "$db" "$ha" step create @sys/step/race --trace "$trace" --required-caller @bob --partial-args "{\"step_id\":\"$onward\"}")" id)
    r2=$(strfield "$(jj "$db" "$ha" step create @sys/step/race --trace "$trace" --required-caller @carol --partial-args "{\"step_id\":\"$onward\"}")" id)

    assert_eq "step_gates.race_first_fires" True "$(resultf "$(jj "$db" "$hb" step complete "$r1" '{}')" fired)"
    assert_json "step_gates.race_onward_done" "$(jj "$db" "$ha" step show "$onward")" status done
    # The loser is not an error: losing a race is the normal outcome for all but one contributor.
    assert_eq "step_gates.race_loser_reports_false" False "$(resultf "$(jj "$db" "$hc" step complete "$r2" '{}')" fired)"

    # Confinement is creator-scoped, not process-scoped. A step parked by a DIFFERENT trace is
    # refused even inside the same process — otherwise anyone funded by the process could fire
    # someone else's parked continuation with input of their choosing (confused deputy).
    local other_seed other_step g_foreign
    other_seed=$(resultf "$(jj "$db" "$hc" run @sys/message '{"to":"@bob","message":"another workflow"}')" step_id)
    other_step=$(strfield "$(jj "$db" "$hc" step show "$other_seed")" id)
    g_foreign=$(strfield "$(jj "$db" "$ha" step create @sys/step/race --trace "$trace" --required-caller @bob --partial-args "{\"step_id\":\"$other_step\"}")" id)
    assert_fails "step_gates.foreign_creator_refused" "unauthorized\|not created by\|permission" -- \
        j "$db" "$hb" step complete "$g_foreign" '{}'
    assert_json "step_gates.foreign_step_untouched" "$(jj "$db" "$hc" step show "$other_step")" status waiting

    # --- join: a barrier of three ---
    local jonward j1 j2 j3
    jonward=$(strfield "$(jj "$db" "$ha" step create @sys/sink --trace "$trace" --required-caller @sys --partial-args '{}')" id)
    j1=$(strfield "$(jj "$db" "$ha" step create @sys/step/join --trace "$trace" --required-caller @bob --partial-args "{\"step_id\":\"$jonward\",\"need\":3}")" id)
    j2=$(strfield "$(jj "$db" "$ha" step create @sys/step/join --trace "$trace" --required-caller @carol --partial-args "{\"step_id\":\"$jonward\",\"need\":3}")" id)
    j3=$(strfield "$(jj "$db" "$ha" step create @sys/step/join --trace "$trace" --required-caller @alice --partial-args "{\"step_id\":\"$jonward\",\"need\":3}")" id)

    assert_eq "step_gates.join_first_waits"  False "$(resultf "$(jj "$db" "$hb" step complete "$j1" '{}')" fired)"
    assert_eq "step_gates.join_second_waits" False "$(resultf "$(jj "$db" "$hc" step complete "$j2" '{}')" fired)"
    assert_json "step_gates.join_onward_still_waiting" "$(jj "$db" "$ha" step show "$jonward")" status waiting
    assert_eq "step_gates.join_third_fires"  True  "$(resultf "$(jj "$db" "$ha" step complete "$j3" '{}')" fired)"
    assert_json "step_gates.join_onward_done" "$(jj "$db" "$ha" step show "$jonward")" status done
}
