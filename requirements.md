...
OAuth-style authorization code + PKCE; CLI device or loopback login; short-lived bearer tokens; rotatable refresh tokens; server-side logout revocation; scripts never receive tokens [→G5]. Recovery: client generates a 12-word BIP-39 mnemonic at `user create`/first boot, derives Ed25519, sends the public half only; `recover/start {handle}` issues a single-use TTL nonce; `recover/complete {handle, nonce, signature, password}` verifies over `{recovery_challenge: nonce}` (domain `recovery`), consumes the nonce, sets the password; under the auth rate limiter [→U2]. Password ≥ 8 chars server-side at create/first-boot/update, no composition rules [policy]. First boot creates the kernel `serve NAME` names: what its configuration does not already say is asked on a terminal and refused off one, naming the key that would have said it — the world, and the chain endpoint where the world has one — and that configuration is written once, the only write of `config.json` there is, so a runtime-only override never reaches disk. It then atomically creates `sys`, `superuser_handle = sys`, an Ed25519 signing keypair and a 32-byte JWT secret, enrolls `sys` recovery and prints the phrase once; partial boot rerunnable. Nothing is created before the name is given or the world is known, so a boot that cannot be answered leaves no half-made kernel. Secrets never logged or returned; `JUICE_SECRET_KEY` runtime-only override. The world digest is recorded once the rail has verified the world, never before, so a chain that is not what it claims binds nothing; every later startup refuses a database whose digest is not the configured world's, naming the network the database belongs to, and a database that predates the rail is bound to `play` alone, so credits by fiat never become claims on a token. A database recording a migration newer than any this build ships is refused rather than read under a schema the binary does not know. One server per kernel home holds an exclusive lock, so one signer and one set of workers; an unnamed legacy home moves into the named home by one rename before that lock is taken, refused rather than merged if a server holds it or the destination exists. A chain world creates its rail key once, on disk before it is named and never overwritten. Every startup: confirm first boot via `superuser_handle`, verify keys (abort if absent), register/enable natives + reconcile config, soft-delete unregistered natives, run recovery; bootstrap idempotent. [→U1–U2, U43]
...
a first boot refuses rather than choose what a kernel cannot revise: no world configured and no terminal to ask on creates no database, an unknown configuration key is refused naming the key, and the world digest is recorded only after the rail has verified the world
