...
| `DiscoveredKernel`  | `public_key`, `handle`, `about`, `gossip_cursor`, `first_seen`, `updated_at`, `last_attempt_at`, `attempts`                                                                                                                                                                                    | One row per kernel; a regenerable discovery cache accumulated from gossip (§13). `about` is the kernel's self-description; `gossip_cursor` is the persisted evidence high-watermark for that peer (§13 peer sync). `last_attempt_at`/`attempts` are the PEX pull-attempt state (§13): `last_attempt_at` orders the pull rotation least-recently-attempted first, `attempts` counts consecutive non-verified pulls (reset on a verified one), and a never-verified stub (empty `handle`) is evicted once `attempts` crosses a bound — so a poisoned hint cannot linger. Information only — never execution semantics, callability, pricing, or settlement. Location is not stored; the transport resolves a key when needed (§13).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
**Identity is a key; names are local.** A stable principal is `PrincipalID = (kernel_public_key, user_id)`; neither half changes when a handle changes. Human-facing references use *local* names — `owner/name` locally, `owner@kernel/name` across kernels, where `kernel` is a **local alias** for a remote key and a handle is bare (§14 grammar). `owner@<kernel-key>/name` (raw key) always works; resolution captures the underlying `PrincipalID` beneath the friendly name (on the proxy row's `remote_owner_id`, a Step's `required_caller_remote_id`, and receipts). A local alias is **presentation only**: bound automatically at first meaningful use (below), sticky to the key, collision-suffixed, renamable with `admin rename`; a later change to the remote's advertised label never retargets it. Two owners on B never collide (`alice@B/greet` vs `bob@B/greet`), yet both settle into B's single proxy user — the one bilateral wallet, named and attributed but not fragmented. Key rotation is unsupported: a lost key is a lost identity and reachability at once.

### Calling a remote action — resolve on use

Consuming another kernel's actions requires **no subscription** — naming `owner@B/name` resolves and caches that one action on demand (§8), a purely local read to which B makes no decision. **First meaningful use** — the first verified action resolve that creates a proxy row, or the first verified remote-user resolve for a Step — is also when A binds a local **alias** for B's key: B's advertised label when a valid one is already known (discovery table or prior gossip pull) or obtainable by a *best-effort* pull that can never fail or delay the call, else the mechanical `k-<key8>`; collisions suffix locally (`acme2`). Discovery alone binds nothing, and an advertised label **never resolves a reference** — only a bound alias or a raw key does — so no kernel captures `owner@acme/name` by gossiping the label `acme` first. A cold call by raw key therefore always succeeds even when no friendly label is available.
...
**Peer exchange and sync.** Discovery and peer sync are **one loop, one rule** (`discovery_interval_seconds`, §14): each pass pulls gossip from every kernel it knows — its peers, the `bootstrap_peers` seeds, and a rotation of kernels learned from `known_kernels` hints — verifying each first-party. A pull is **verified** only when it is authenticated as the key dialed (the reply's `public_key` equals it) and carries a valid bare handle; a verified pull refreshes the `DiscoveredKernel` row (making the kernel eligible to be relayed as a hint) and resets its attempt count. Every other outcome — unreachable, malformed, key-mismatch, or invalid identity — is a failed attempt that rotates the candidate to the back (least-recently-attempted is pulled first) and, for a never-verified stub, evicts it after a small bound; the unverified cache is capped. So a poisoned or dead hint costs at most a few pulls and converges out, and knowledge of a kernel reaches the network progressively as hints propagate. A successful pull from a known, non-suspended peer additionally persists `peer_last_seen = now` and `peer_credit = counterparty_balance` on the peer's user row (§3): a display-only cache (§14) that never gates a call, price, or settlement, and never counts as peer activity for retention (§13 Retention) — answering gossip is free liveness and would otherwise immortalize a zombie peer; retention stays trade- and value-based.
...
gossip serves identity, first-party users (sys + public-action owners), own signed manifests, and one evidence page per pull with the catalog snapshot on every reply; evidence is never relayed (learned evidence is not re-gossiped); identity hints (`known_kernels`) are keys only, bounded, and only kernels self-verified within the horizon; keyed by public key with no URLs
gossip relays only a bounded sample of self-verified-within-horizon kernels, excluding stubs, stale rows, self, and the requester; the horizon derives from the discovery interval
`known_kernels` hints ingest as unverified stubs (own/sender/syntactically-invalid skipped, capped) and never clobber a verified row; an authenticated gossip requester is learned as a stub, never provisioned an account; a hinted stub is pulled a later pass and, if it verifies, indexed
a gossip pull is verified only when authenticated as the dialed key (reply `public_key` matches) with a valid bare handle; a key-mismatch or invalid-handle reply is a failed attempt (rotated back, counting toward stub eviction), never accumulated
a never-verified stub is evicted after a bounded number of failed pulls and the unverified cache is capped, so a poisoned or dead hint cannot pin or grow the pull set
...
a kernel rates a call to a remote action, so it gossips trade evidence about that subject; a third kernel
  reaches it via peer-exchange (bootstrapped off the seed, it learns a non-bootstrap kernel from a
  `known_kernels` hint, then pulls it directly), discovers the subject's action via its discovery cache
  (sys/lookup), resolves the subject's action directly by key, runs — its own Stats start at defaults and accumulate
