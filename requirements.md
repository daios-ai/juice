...
| `GET /v1/grants/plan?selector=`                  | authenticated; expands the selector (§8) over delegated actions the caller may call and returns the consent plan `user connect` walks: groups by `provider_key` — `provider`, scheme, requested scope union, connected/covered status, and per-action `granted` flag — plus counts of skipped actions (loginless or not-callable) |
...
connection provider_key derived from the pinned base-URL host (bearer) and token_url|client_id (oauth), never from action names
