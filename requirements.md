...
manual enable/disable on a remote_proxy action is rejected with ErrInvalidState
...
a kernel rates a call to a remote action, so it gossips trade evidence about that subject; a third kernel
  ...
  resolves the subject's action directly by key, runs — its own Stats start at defaults and accumulate
