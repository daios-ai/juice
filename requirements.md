...
The proxy's local `price` is `manifest.price` plus the provider's advertised remote premium: `price = mp + ceil(mp * manifest.remote_bps / 10000)` (§13). `remote_bps` is set by the **provider** kernel and carried in the signed manifest, so the local caller sees one authenticated price bounding the whole remote call, premium included; settlement charges the premium on the actual remote charge and refunds the difference (§13). Because `remote_bps` is a signed manifest field, a change to it *is* a contract change (re-synced on the next resolve), unlike the retired caller-side import duty it replaces.
...
Federation protocols are versioned libp2p streams: `/juice/fed/call/1` (inbound proxy call), `/juice/fed/step/1` (step listing and completion, below), `/juice/fed/resolve/1` (open read-only single-action manifest and principal resolution — the execution primitive: resolve one action or one user by handle→`PrincipalID`, so calling needs no subscription), `/juice/fed/manifest/1` (bulk manifest serving for `admin subscribe`, chunked per action so a large-catalog warm survives bandwidth-capped relayed connections), `/juice/fed/gossip/1`, and `/juice/fed/inspect/1`. `Call` enters a computation and `CompleteStep` resumes one (§10) — a wire carrying only the first would strand every continuation addressed across a boundary. There is no subscribe handshake — a call resolves and caches one action locally (below). Payloads and verification are exactly the settlement rules below; only the carrier is libp2p. The v0.12 wire (stable `PrincipalID`, `owner_id`/`remote_bps`/`max_price` in signed payloads, `/juice/fed/resolve/1`, step attestation) is the scheduled protocol-version break: a v0.12 kernel does not interoperate with a pre-0.12 peer on the changed surfaces.
...
juice admin credit <user> --max N [--settle-at M]   set the bilateral credit this kernel extends the peer (provider side, §13)
...
juice admin peers                       known peers, bilateral balance, credit max/used, settlement state (--all also shows suspended)
...
Calling is **permissionless**: a kernel's public actions are by definition callable, so resolve needs no approval, pending, or handshake. Payment is the trust decision, and it is **bilateral** (§13 credit). The peer account's `available` is the bilateral position — positive when the peer prepaid, negative when the peer owes this kernel, bounded below by `−peer_credit_max`. A caller with no account is auto-provisioned a zero-balance peer account on its first signed call, so a price-0 action runs at once; a priced one runs against prepaid balance and/or against bounded credit the **provider** extends (`admin credit <peer> --max N`, §14), and is refused with a signed 402 rejection once the peer's debt would exceed `credit_max`. Operators settle bilateral positions out of band and record them with the existing `deposit`/`withdraw` primitives (§12): `deposit` reduces what the peer owes this kernel (or prepays it forward), `withdraw` redeems what this kernel owes the peer — Juice owns the ledger, external systems move the money. The local alias is addressing only, never signed or sent.
...
The relation is **not transitive**: invoking B's actions requires resolving them from B directly (a `remote_proxy` cache row is never re-served, §8). Exposure is each side's lever: a provider may refuse further paid calls once a peer's debt reaches `credit_max`; settlement or a deposit restores them.
...
### Money — bilateral credit

Federation money is the **bilateral peer account**, not a prepaid-only balance. The peer `User`'s `available` is the bilateral position between the two kernels: from B's side, B's account for A is positive when A prepaid (B owes A), zero when settled, and negative — down to `−peer_credit_max` — when A owes B. A caller therefore need not prepay: A's users call B's actions immediately, drawing prepaid balance if any and otherwise bounded credit the **provider** B extends (`admin credit`, §14). Execution never mints or burns beyond that bounded credit line; the only unbounded money movements remain supervision (`Deposit`/`Withdraw`).

The account nets automatically because it is one row: B's balance for A rises when A's users call B (settlement pays it `charge + premium` as `target_user_id`, §6/§13) and falls when B's users call A (inbound calls spend it, possibly into credit). Operators settle only the net, out of band, and **record it with the existing primitives** — `Deposit` reduces what the peer owes (or prepays forward), `Withdraw` redeems what this kernel owes the peer — keyed idempotently by `external_key`. Juice owns the ledger and the obligation; the external rail (bank, card, L2, stablecoin, manual) moves the real money and is out of Juice's scope. The provider bears counterparty risk, bounded by `credit_max`: keep it small and settle at the `settlement_trigger`.

**Credit admission and settlement trigger.** Before executing an inbound paid call, the provider atomically reserves the call's worst-case cost — `mp + maxpremium` — against the peer account, admitting it iff the resulting position stays `≥ −credit_max` (concurrent calls cannot together breach the limit, since the reservation is the atomic wallet move of §6). Over-limit is refused with a signed zero-charge 402 rejection, surfaced to the caller as `ErrPeerUnfunded` (an operator condition — settle or deposit — never the caller's own insufficient funds). When the peer's debt reaches `settlement_trigger` the provider flags `settlement_due` and advertises it to the debtor over the authenticated gossip response (information, never authority; the debtor's operator sees it on `admin peers`); crossing the trigger does **not** stop execution — only reaching `credit_max` does. The exact rail, netting cadence, collateral, and credit-assessment policy are separate concerns; the kernel guarantees only that unpaid exposure is bounded and every cross-kernel debt is a signed-receipt-backed obligation settled out of band.

### Calls, receipts, settlement

Outbound: a remote-proxy call follows normal role law (`owner` = local process owner, `caller` = local call caller, `target` = the proxy user) and normal wallet mechanics, funded with the proxy's local price `mp + maxpremium`, where `maxpremium = ceil(mp * remote_bps / 10000)` and `remote_bps` is the **provider's** advertised premium (a signed manifest field, §8). The handler records the UUID v4 `idempotency_key` and the outbound request payload (`dispatch_json`, which snapshots `remote_bps` and the accepted `max_price`) on the proxy trace atomically with dispatch (§3, §5) — the stored payload is what makes retry after restart possible. On the remote kernel it arrives over the `/juice/fed/call/1` protocol, authenticated by the per-request federation signature (not a bearer token), and is then an ordinary inbound call by this kernel's proxy user there, paid from its **bilateral balance** — positive when prepaid, negative within the provider's `credit_max` (§13 credit), executed wholly under that kernel's §6.

A proxy call settles **only on a signed remote receipt** — never on a network timeout. The premium is charged by the **provider** and carried in the receipt (`receipt.premium`, computed on the *actual* charge); the caller kernel earns nothing on it:

```text
remote success:   charge   = receipt.charge (= receipt.base_charge = mp)
                  premium  = receipt.premium (= ceil(charge * remote_bps / 10000))
                  pay charge + premium to the proxy user (the bilateral payable to the peer)
                  refund (mp + maxpremium) − charge − premium to the caller's trace
...
remote failure (charge = receipt.charge ≤ mp; settled remote descendants stay paid, §6):
                  ...
                  pay charge + premium to the proxy user
                  refund (mp + maxpremium) − charge − premium to the caller's trace
                  local transaction: failure
signed rejection (charge = 0, premium = 0):
                  full refund; local transaction: failure
                  a rejection whose transport status is 402 (the remote will extend no further
                  credit — this kernel's bilateral debt there has reached credit_max) settles as
                  ErrPeerUnfunded — an operator condition (settle or deposit out of band),
                  never rendered as the caller's own insufficient funds
...
The premium is the **provider's** compensation for servicing and financing the remote call, credited to the provider's `sys` — a caller kernel earns nothing on remote calls it makes (contrast the local layer fee, §6). The provider's `remote_bps` (default 500) is a signed manifest contract field; the caller can never be charged more than the `max_price` it authenticated and locked before dispatch (§8). Forced closure (`EndProcess`) fails an in-flight proxy call like any running call (§6) and refunds the caller; if the remote side did commit, its charge stands there and is absorbed by the bilateral account, surfacing at reconciliation.
...
proxy call locks mp+maxpremium; success settles charge+premium (premium from the receipt, on actual charge) and refunds the difference
remote failure with charge refunds (mp+maxpremium)−charge−premium; settled remote work stays paid
failure counts against proxy stats regardless of charge
provider premium credited to the provider's sys; charge+premium credited to the proxy user (the bilateral payable); caller kernel earns nothing on remote calls
...
```text
ordinary-account balances are never negative; a peer account may go negative only within its `peer_credit_max` (DB CHECK `available >= -peer_credit_max`), and `locked` is never negative
...
Federation incentives align in both directions: a kernel federates because its users gain a larger action space and its providers gain outside demand, its operator earning the **provider premium** (`remote_bps`) on inbound remote calls — compensation for servicing and *financing* those calls, since the provider extends bounded credit and carries the float until settlement. The premium accrues to the *provider's* `sys` (contrast the retired import duty, which paid the importing kernel that bore no risk); this places the reward on the party that actually takes on the credit exposure and default risk, and lets each provider price its own risk through `remote_bps`, `credit_max`, and `settlement_trigger`. Debtors reduce cost by prepaying (a deposit shrinks the negative position they draw on), and neither side can move the price after signature (the caller locks the authenticated `max_price`). Discipline is self-enforcing: a kernel whose bilateral debt reaches a peer's `credit_max` is refused further paid calls until it settles, and executing unpaid work would lose money twice — in service and in the failure stats that sink its rank abroad. Because gossip carries only trade-backed opinions, reliable behavior compounds into discoverability: reputation is the long-run asset earned by settling honestly.
