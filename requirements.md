...
OpenAPI import --as names every operation under one prefix and the operation keyed index becomes the group's root, reachable as owner/prefix; re-importing one spec_url under a different prefix is refused with the rows left in place, and a prefix carrying @ or an empty segment is refused
