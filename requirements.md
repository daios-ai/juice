...
| `Grant`             | `id`, `grantor_user_id`, `action_id`, `refresh_token`, `created_at` | A user's delegated upstream credential for exactly one action (§8): an OAuth refresh token (`oauth_delegated`) or a static bearer/API-key token (`delegated_bearer`), per the action's scheme. Unique per `(grantor_user_id, action_id)` — a re-consent overwrites in place. `refresh_token` is AES-256-GCM encrypted at rest and write-only: never returned by any read path. Deleted on revoke, on provider `invalid_grant`, and when a deactivating update, auth replacement, or delete invalidates the action (§8). A consent record, not an authentication credential. |
...
| `POST /v1/grants`                                | authenticated; `{action, token}` stores a static token for a `delegated_bearer` action (§8) — the non-OAuth direct twin of start/complete; rejects a non-`delegated_bearer` action and an action the caller may not call |
...
a user attaches a per-user API key to a delegated_bearer action via `user connect --token`, runs it
  and the fake upstream sees the token in the configured header; sees the grant in `user me`;
  disconnects; the next run rejects pre-lock with grant_required
