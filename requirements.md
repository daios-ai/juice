...
| `Trace`             | `id`, `process_id`, `parent_trace_id`, `action_owner_id`, `available`, `locked`, `idempotency_key`, `dispatch_json`, `premium_bps`, `premium_parked`, `value`, `value_to`, `created_at`                                                                                                                                  | `action_owner_id` is the owner of the action executing in the trace, used for trace-scoped process authority. `premium_bps`/`premium_parked` snapshot the serving markup admitted for an inbound federated root call (§13): the rate the receipt levies on the actual charge, and the reserve parked in the owner's `locked` at admission. `value`/`value_to` snapshot a federated value transfer's delivered amount and local beneficiary, parked alongside the premium reserve and credited to the beneficiary at settlement. They ride on the trace so **every** settlement path — commit, failure, crash recovery, forced closure, max-age — releases the reserve without the in-memory request and pins the rate against a mid-call config change; all are 0 on local calls and subcalls. `process_id` is denormalized (derivable by walking `parent_trace_id` to the root). Root traces have null parent. Every `Call()` creates exactly one child trace. A trace is the call's wallet: `available` starts as the action's price at entry (the call's remaining allocation); `locked` is what the call has committed to direct subcalls and steps. Calling price `q` requires `available ≥ q` and moves `q` from `available` into `locked`, becoming the callee's `available` (§6). Settlement pays out the trace's remaining `available` (§6). Own latency is `transaction.ended_at − transaction.started_at`; no cached latency field (§11). `idempotency_key`/`dispatch_json` are null except on a remote-proxy trace, where the outbound key and request payload are recorded atomically with dispatch; `idempotency_record_id` is the *inbound* cross-kernel record the trace serves when the call is answering a peer (§13) — set for every action kind, so whichever settlement finally resolves the trace (commit, dispatch retry, max-age bound, forced closure, or crash recovery) completes that record; while set and unsettled, the call is awaiting its receipt and restart resumes its retry (§5, §13). |
...
**Subscription is not a prerequisite for calling.** A cold call to a kernel-qualified action (e.g. `alice@B/foo`, §13 grammar) resolves that one action's signed manifest on demand over `/juice/fed/resolve/1` (§13), verifies it, and creates or refreshes a local `kind=remote_proxy` **cache row** owned by the local remote-peer user row — then dispatches. `admin subscribe` is the *optional bulk* form of the same operation: it warms the cache with the peer's whole active public catalog at once. Either way the resulting proxy rows are enabled and given `visibility = local`: callable by this kernel's own users but never re-served to a further peer, so federation stays non-transitive at the call layer as well as the manifest layer — a peer that names one of your imported proxies is denied by `CanCall` (a peer caller fails the `local` branch, §4). Neither form copies implementation. The proxy row is a **transitional cache**, reconstructible from `(peer_key, owner_id, remote_action_id, contract)`; it carries no semantics of its own — ownership, attribution, and reputation key on the global `PrincipalID = (peer_key, owner_id)` (§13), never on the local proxy user.
...
`owner_id` is the action owner's **stable** id on the serving kernel (the identity half of `PrincipalID`); `owner_handle` is that owner's current *display* handle there (mutable metadata, never identity). Only active public remote actions have manifests; an action using a delegated auth scheme (`oauth_delegated` or `delegated_bearer`, §8) is never served as a manifest and never gossiped, because a remote peer's single proxy user can never complete a browser consent nor hold a per-caller token. A kernel serves manifests and gossips (as its own exposed actions) only actions it owns — `kind ∈ {http, wasm, native}`; an imported `kind=remote_proxy` action is never re-served, so federation stays non-transitive: reaching a peer's imported action requires resolving/subscribing to its true owner directly. Manifest descriptions and schemas are the canonical interface used by importing kernels for lookup and LLM function calling. `signature` is the remote platform Ed25519 signature over canonical JSON excluding `signature`, verified against the remote peer's `public_key`. Match key (stable-identity based, so a remote handle rename never re-keys the cache):
...
```text
action_id artifact_hash description input_schema kind name output_schema owner_id price remote_bps
```

`owner_handle` is **not** a contract field — a rename on the remote updates display only and never deactivates the proxy. Manifest stats and `updated_at` do not affect contract comparison; manifest stats never overwrite local `Stats` and are not stored as `StatTag`. Invalid signatures skip that action. A re-resolve or a second `admin subscribe` re-syncs: new actions are imported, changed-contract actions are updated (re-enabled), and actions no longer active/public on the remote are deactivated and their stats reset. `admin unsubscribe` deactivates all proxies from that peer and preserves all history.

The proxy's local `price` combines two markups on the base price `mp` (§13 pricing): the **serving kernel's** advertised remote markup (execution tax + risk premium, one number `remote_bps`, a signed manifest field) and the **origin kernel's** import fee (`import_bps`, local origin policy, retained locally). `sr = mp + ceil(mp * remote_bps / 10000)` is what the origin will owe the serving kernel; `price = sr + ceil(sr * import_bps / 10000)` is what the local user pays. The user sees one authenticated, deterministic price bounding the whole remote call. `remote_bps` is a signed manifest contract field (a change re-syncs on the next resolve); `import_bps` is local origin policy, so changing it re-prices proxies **without** a manifest change. Settlement charges the serving markup on the actual remote charge and the import fee on the actual obligation, refunding the difference (§13).
...
| `sys/transfer` | Public; action owner `sys`; price 0 (configurable, `native.transfer`, §14); callable through `Call()`. Moves `amount` credits from the immediate caller to `target` (§13 value transfer). Input: required `target` (a local `handle`, or a bare handle on the far kernel when calling `sys@<kernel>/transfer`), required `amount` (positive integer), optional `external_key` (idempotency, local transfers). Output: `amount`, plus `transfer_id` for a same-kernel transfer. A same-kernel target settles as one atomic ledger entry (no fee); a cross-kernel target rides the call value-bearing through admission/receipt/settlement. `ErrInvalidInput` for a non-positive/fractional amount, a kernel-qualified `target` on the local action, or a peer/remote beneficiary the caller may not reach. |
...
Federation protocols are versioned libp2p streams: `/juice/fed/call/1` (inbound proxy call), `/juice/fed/step/1` (step listing and completion, below), `/juice/fed/resolve/1` (open read-only single-action manifest and principal resolution — the execution primitive: resolve one action or one user by handle→`PrincipalID`, so calling needs no subscription), `/juice/fed/manifest/1` (bulk manifest serving for `admin subscribe`, chunked per action so a large-catalog warm survives bandwidth-capped relayed connections), `/juice/fed/gossip/1`, `/juice/fed/settle/1` (two-party commit/reveal for sub-quantum residual debts, §13), and `/juice/fed/inspect/1`. `Call` enters a computation and `CompleteStep` resumes one (§10) — a wire carrying only the first would strand every continuation addressed across a boundary. There is no subscribe handshake — a call resolves and caches one action locally (below). Payloads and verification are exactly the settlement rules below; only the carrier is libp2p. The v0.12 wire (stable `PrincipalID`, `owner_id`/`remote_bps`/`max_price` in signed payloads, `/juice/fed/resolve/1`, step attestation) is the scheduled protocol-version break: a v0.12 kernel does not interoperate with a pre-0.12 peer on the changed surfaces.
...
**Identity is a key; names are local.** A stable principal is `PrincipalID = (kernel_public_key, user_id)`: the kernel key names the kernel in the global namespace, `user_id` the account within it, and neither changes when a handle changes. Human-facing references use *local* names — an action is `owner/name` locally and `owner@kernel/name` across kernels, where `kernel` is a **local alias** for a remote kernel key, `@` qualifies a kernel, `/` namespaces an action, and a handle is bare (§14 grammar). `owner@<kernel-key>/name` is an escape hatch that always works — it looks the owner up by their *current* handle on that kernel — but it is not itself the stable identity: resolution captures the underlying `PrincipalID` and records it beneath the friendly name (on the proxy cache row's `remote_owner_id`, on a Step's `required_caller_remote_id`, and in receipts). A local alias for a remote kernel is **presentation only**, never identity: it is bound automatically at first meaningful use (below), sticky to the key, collision-suffixed, and renamable with `admin rename` — a later change to the remote's advertised label never renames or retargets it. Two owners on B with the same action name never collide (`alice@B/greet` vs `bob@B/greet`). Settlement rolls up to the one peer account for B (the bilateral wallet): every one of B's owners settles into B's single proxy user, so `alice@B/…` and `bob@B/…` name and attribute but do not fragment the wallet. Key rotation is unsupported — a lost key is a lost identity and reachability at once.

### Calling a remote action — resolve, don't subscribe

Consuming another kernel's actions requires **no subscription**. A names a remote action `owner@B/name`; on first use A resolves that one action's signed manifest from B over `/juice/fed/resolve/1` (open, read-only), verifies it, caches a `visibility=local` proxy row, and calls it. `subscribe` is only the *bulk* form of the same operation — it warms the cache with B's whole active public catalog at once — and, like resolve, is a purely local read that writes nothing to B; B makes no decision. **First meaningful use** — the first verified action resolve that creates a proxy row, or the first verified remote-user resolve for a Step — is also when A binds a local **alias** for B's key: it uses B's advertised label when a valid one is already known (from the discovery table or a prior gossip pull) or obtainable by a *best-effort* identity pull that can never fail or delay the call, else the mechanical `k-<key8>`; collisions suffix locally (`acme2`). Discovery alone binds nothing, and an advertised label **never resolves a reference** — only a bound local alias or a raw key does — so no kernel can capture `owner@acme/name` by gossiping the label `acme` first. A cold call by raw key therefore always succeeds even when no friendly label can be obtained.

```text
juice admin subscribe <key>             optional: bulk-cache a peer's whole active public catalog (a call resolves one action on demand without this)
juice admin unsubscribe <user>          stop consuming: deactivate that peer's cached proxies here (user is handle or key)
juice admin settle <user>               settle the bilateral position with a peer: exact if debt ≥ Q, else the probabilistic residual protocol (§13)
juice admin suspend <user>              freeze any account — local or peer — so it cannot act
juice admin unsuspend <user>            lift a suspension
juice admin rename <user> <new-handle>  rebind an account's local handle, including a peer's local alias
juice admin peers                       known peers, bilateral balance, settlement state (--all also shows suspended)
...
Calling is **permissionless**: a kernel's public actions are by definition callable, so resolve needs no approval, pending, or handshake. Payment is the trust decision, and it is **global exposure** (§13). The peer account's `available` is the bilateral position — positive when the peer prepaid, negative when the peer owes this kernel; a peer row is bounded not per-peer but by the kernel's **global** exposure cap `X`, enforced at admission across all peers combined (§13, Sybil-proof). A caller with no account is auto-provisioned a zero-balance peer account on its first signed call, so a price-0 action runs at once; a priced one runs against prepaid balance and/or against the serving kernel's global exposure headroom, and is refused with a signed 402 rejection once admitting it would push gross receivables above `X`. Operators settle positions out of band and record them with the existing `deposit`/`withdraw` primitives (§12): `deposit` reduces what the peer owes this kernel (or prepays it forward), `withdraw` redeems what this kernel owes the peer — Juice owns the ledger, external systems move the money. The local alias is addressing only, never signed or sent.

Moderation is **two orthogonal verbs**, one per edge. `unsubscribe` is the consumer's lever: it deactivates the proxies A cached from B and nothing else — A's balance, history, and B's account survive, and a call or `subscribe` re-caches. `suspend` is the account lever, unified across humans and peers (§12): it freezes an account's ability to act — a suspended human cannot log in; a suspended peer's inbound calls are refused with a signed rejection — reversible with `unsuspend`. There is no separate "unfriend" and no `denied_at`: a suspended peer is the former deny, governed by the one `suspended_at` column (§3). Suspend is a pure freeze with no cascade.
...
Juice is one global action market: any user may call any reachable action from their **local** balance, without opening a remote account, prefunding, or negotiating credit on the serving kernel. Users receive deterministic prices and deterministic local deductions; kernels privately absorb, price, cap, aggregate, and settle the resulting financial risk. The peer `User`'s `available` remains the **bilateral position** (positive = the peer prepaid; negative = the peer owes this kernel), netting automatically because it is one row.
...
Outbound: a remote-proxy call follows normal role law (`owner` = local process owner, `caller` = local call caller, `target` = the proxy user) and normal wallet mechanics, funded with the proxy's local price `q = sr + ceil(sr * import_bps / 10000)`, where `sr = mp + ceil(mp * remote_bps / 10000)`, `remote_bps` is the **serving** kernel's advertised markup (a signed manifest field, §8) and `import_bps` is local origin policy (§14). The handler records the UUID v4 `idempotency_key` and the outbound request payload (`dispatch_json`, which snapshots `mp`, `remote_bps`, and `import_bps`) on the proxy trace atomically with dispatch (§3, §5) — the stored payload is what makes retry after restart possible. On the remote kernel it arrives over the `/juice/fed/call/1` protocol, authenticated by the per-request federation signature (not a bearer token), and is then an ordinary inbound call by this kernel's proxy user there, admitted against the serving kernel's global exposure cap `X` (§13), executed wholly under that kernel's §6.
...
remote success:   charge    = receipt.charge (= receipt.base_charge = mp)
                  ...
                  paid      = charge + premium                  → proxy user (bilateral payable to the peer)
...
remote failure (charge = receipt.charge ≤ mp; settled remote descendants stay paid, §6):
                  ...
                  paid      = charge + premium                  → proxy user
...
signed rejection (charge = 0, premium = 0):
                  ...
                  a rejection whose transport status is 402 (the serving kernel's global
                  exposure cap X is exhausted) settles as ErrPeerUnfunded — an operator
                  condition (settle or deposit out of band), never rendered as the caller's
                  own insufficient funds
...
`sys/transfer(target, amount)` moves `amount` credits from the immediate caller `C` (§1 role law — never an argument, never the process owner) to `target`. It is a native action (`kind=native`, `price=0`, `visibility=public`) that routes user→user payment through `Call()`, so caller and Step semantics provide authorization without a second money primitive: composed as a subcall it pays from the composing action's owner; parked in a Step it pays when the required caller completes it.

A **same-kernel** target settles as the existing atomic ledger transfer (one `CreateLedgerEntry`, no fee, `external_key` idempotency); a peer/proxy row is rejected as a recipient — it is a financial counterparty, never a semantic recipient. A **cross-kernel** transfer is addressed like any remote action, `sys@<kernel>/transfer` with a bare `target` on that kernel (the one addressing convention; a kernel-qualified `target` on the local action is rejected). It makes the call **value-bearing**: the amount rides the call through admission, reservation, receipt, and settlement, so both kernels' books move only on signed-receipt evidence.

The origin funds `q = proxyGross(mp + value)` — the two-step markup on the base price plus the delivered value — locked reserve-first and finalized only by a signed receipt (refunded on a signed rejection, parked and retried after an uncertain dispatch, exactly the remote-call rules above). The serving kernel admits `W = price + value + ceil((price+value)·remote_bps/10000)` against its global exposure `X` (a transfer is unsecured lending until settlement, which is what `X` bounds), resolves the beneficiary to a local ordinary active account at admission (an unknown/suspended/peer beneficiary is a signed zero-charge, zero-value rejection), and **parks the value beside the premium reserve** — released to the beneficiary at settlement, never through `ComputeFee`, so it is delivered untaxed. The receipt separates execution from delivery: `charge` = execution consumed (unchanged, partial on failure), **`value`** = amount delivered, all-or-nothing (`value ∈ {0, amount}`; a receipt claiming partial value is quarantined). The bilateral obligation is `paid = charge + value + premium`, `premium = ceil((charge+value)·remote_bps/10000)` (the serving kernel fronts the value and carries its credit risk, so the markup applies to both); the origin retains `import_bps` on the whole obligation uniformly (policy, no transfer exemption — an exemption would be an arbitrage channel). The sender pays all fees on top; the beneficiary receives exactly `amount`. `VerifyRemoteReceipt` extends by folding value into the charge and premium checks (`tx.net = charge + value + premium`). The relation stays non-transitive: a peer caller naming a remote beneficiary is rejected.
...
admin rename rebinds a peer's local alias to an operator-chosen handle (actions addressable and callable under it); a colliding alias auto-suffixes
...
serving markup (premium) credited to the serving kernel's sys; charge+premium credited to the proxy user (the bilateral payable); origin retains its import fee to origin sys; caller kernel earns only that local fee, never the premium
...
sys/transfer moves credits from the caller C to target; a same-kernel target settles as one atomic ledger entry (no fee), a remote target (sys@kernel/transfer) rides the call value-bearing
a value transfer funds q=proxyGross(mp+value); the serving kernel admits W=price+value+premium against X and delivers value to the beneficiary at settlement (parked, untaxed), never via the handler on the peer path
receipt.value ∈ {0, amount} (all-or-nothing); bilateral paid = charge+value+premium; premium=ceil((charge+value)·remote_bps); a receipt short-changing value or mispricing premium is quarantined
value transfer is non-transitive (a peer caller naming a remote beneficiary is rejected); the serving markup applies to value; the origin import fee applies to the whole obligation uniformly
...
A subscribes to B by key; B's operator deposits A's proxy by key (provisioning + funding it);
  A imports B's action; A's user runs it; charge+premium lands in A's proxy balance on B, premium to B's sys,
...
Federation incentives align in both directions: a kernel federates because its users gain a larger action space and its providers gain outside demand, its operator earning the **serving markup** (`remote_bps`) on inbound remote calls — compensation for servicing and *financing* those calls, since the serving kernel extends unsecured credit (bounded globally by `X`) and carries the float until settlement. The markup accrues to the *serving* kernel's `sys`, placing the reward on the party that actually takes on the credit exposure and default risk, and lets each kernel price its own risk through `remote_bps` and its exposure policy (`X`, `Y`, `Q`); the origin separately retains its `import_bps` import fee to cover its outgoing rail cost and variance. Debtors reduce cost by prepaying (a deposit shrinks the negative position they draw on), and neither side can move the price after signature (the caller locks the authenticated local price). Discipline is self-enforcing: a kernel whose share of a serving kernel's gross receivables would breach `X` is refused further paid calls until it settles, and executing unpaid work would lose money twice — in service and in the failure stats that sink its rank abroad. Probabilistic residual settlement keeps sub-quantum dust debts from either accumulating as unbounded exposure or being written off (which would invite dust default); it is EV-exact (`E[payment] = d`), so long-run solvency needs no write-offs — only reserves against variance whose expectation is zero. Because gossip carries only trade-backed opinions, reliable behavior compounds into discoverability: reputation is the long-run asset earned by settling honestly.
