...
- **Probabilistic** (`0 < d < Q`): pay `Q` with probability `d/Q`, else `0`. `E[payment] = (d/Q)·Q = d`, so it is EV-exact; a completed outcome clears the debt either way, and the variance is absorbed by kernels (never users). No deterministic option satisfies all constraints (paying `d` is uneconomical; paying `Q` overpays; forgiving enables dust default; leaving it outstanding consumes `X`; a minimum action price of `Q` destroys micropayments), so probabilistic residual settlement is required.
...
**Three-way accounting.** A completed settlement records three effects so reserves and profitability reconcile, satisfying the per-kernel identity `Δbilateral_row + Δoperator_sys − external_cash = 0`:

```text
creditor (was owed d):  clear peer row +d;  sys += (Q−d) on pay / −d on clear;  cash_in  Q / 0
debtor   (owed d):      clear peer row −d;  sys −= (Q−d) on pay / += d on clear; cash_out Q / 0
```

The debt `d` clears on the peer/proxy row; the external `Q`/`0` is the rail move (recorded on **pay** with `deposit`/`withdraw --external-key <settlement_id>`); the residual is realized **operator variance** on `sys` (`Q−d` gain on pay, `−d` loss on clear). `E[variance] = (d/Q)(Q−d) + (1−d/Q)(−d) = 0`, so variance affects required reserves but not expected value — kernel operators therefore hold reserves, and long-run solvency does not depend on writing off residues.
...
inbound paid call admitted iff global gross receivables G=Σmax(0,−available) stays ≤ X after reserving the worst-case obligation; X=0 ⇒ prepaid-only; two peer identities cannot jointly exceed one X (Sybil-proof)
exposure admission is atomic under concurrency: parallel inbound calls cannot together breach X
probabilistic residual settlement pays Q w.p. d/Q else 0 for 0<d<Q; a completed outcome clears the debt d either way; E[payment]=d
settlement is three-way per kernel: Δbilateral_row + Δoperator_sys − external_cash = 0; sys variance is Q−d on pay / −d on clear; E[variance]=0
