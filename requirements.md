...
`sys/transfer(target, amount)` is an **ordinary priced action with one privileged, deferred, receipt-backed transfer effect**. Its execution channel is the normal `Call()` lifecycle (charge/premium/import on any operator-configured `price`, `native.transfer`, §14); its value channel is an additive effect that moves `amount` credits from the immediate caller `C` (§1 role law — never an argument, never the process owner) to `target`. The two channels are funded from **different wallets** and never mix: the **execution price** comes from the trace (process budget, owner `P`) and is taxed by `ComputeFee`; the **value** comes from `C`'s own `available` and is delivered untaxed. For a root call `C = P` so the wallets coincide; under composition they diverge — a subcall pays the value from the composing action's owner (a marketplace pays the seller from its own funds), and a value parked in a Step pays when the required caller completes it. Routing payment through `Call()` means caller and Step semantics supply authorization with no second money primitive.
...
**Payment steps.** A Step whose action bears the transfer effect (§13 value transfer) is a *payment step*: a remote buyer completing it funds the value channel. The serving kernel advertises a `payment` descriptor on the step listing — `{beneficiary, amount, remote_bps, remote_max, hash}`, where `remote_max = amount + value_premium` is the bilateral obligation the serving kernel can compute (it carries only the remote obligation, never the buyer's `value_import`/`max_total`, which are the buyer's local policy). The buyer funds `max_total = remote_max + value_import` reserve-first from its OWN balance into a dedicated `pending_transfers` record (not a fabricated local trace — it is not executing a local Call), completes, and settles on the serving kernel's signed receipt: on a valid success `value + value_premium` credits the serving-kernel proxy row (the buyer owes it) and `value_import` its own `sys`; a valid failure refunds the whole reserve; an inconsistent/mis-bound receipt keeps the reserve **locked** (quarantined). The payment is bound into the completion **through the idempotency key** — both sides fold the descriptor hash into `idempotency_key = SHA-256("juice/fed/step/1|" + recipient + "|" + step_id + "|" + input_hash + "|" + payment_hash)` (a non-payment step uses `payment_hash = ""`). Since the key is already inside the signed `complete` payload, the payment is transitively bound with no new wire field; the serving kernel recomputes its own descriptor hash and rejects a key that does not match, so a step cannot be completed for a different payment. The serving side stages the inbound transfer effect over the merged args at completion, so admission locks the value reserve from the buyer's proxy row atomically with claiming the step. The beneficiary must be local to the serving kernel (the three-kernel case is out of scope; a protocol account that later pays onward via a direct transfer is the workaround). The step's execution `price` stays creator-parked — the buyer touches only the value channel.

Unresolved `pending_transfers` records (`pending` — no settleable receipt yet — or `quarantined` — a receipt that failed validation) are operator-visible and actionable through one superuser resource (§14): `GET /control/transfers` (default lists the unresolved records; a `status` filter also exposes the terminal `settled`/`refunded`), `GET /control/transfers/{id}`, and `POST /control/transfers/{id}/retry`. **Retry is the only mutation** and never chooses an outcome: it re-presents the SAME signed completion (the stored idempotency key + raw input rebuild the identical request, so the serving kernel replays rather than re-executes) and settles strictly on receipt evidence — a valid success settles, a valid failure refunds, no receipt / an unreachable peer leaves the record pending (a retry has no never-dispatched proof — the first attempt may already have paid the beneficiary), and an invalid receipt stays quarantined. There is deliberately no `refund`/`force-settle`/`edit`/`delete`: quarantine means "evidence is insufficient", reconciled out of band, not "the operator may pick a result". The same disposition method serves any future automatic sweeper, so the command is a manual trigger, not a second settlement path.
