...
- **U3** The operator can credit a user's balance only against a finalized payment from outside — one that reached the kernel's own rail account, or, where the world has no rail, the operator's own record of it; the same payment never moves money twice. Money leaves only by its owner's withdrawal (U51). The kernel owns the ledger; the rail moves the real money.
...
- **Seller.** An obligation closes only against a finalized payment from the payer frozen at admission, for the amount drawn, carrying the transaction named; one payment closes one obligation. A buyer that never reveals is bounded by the credit limit like any other.
- **Ledger.** The budget pays `D + importfee`, `D` returns to C, C's stake carries the draw; cash crosses only when a ticket pays, `C −L` out of the buyer's operator hold, `L` in and whole to the provider. No operator keeps any part of a draw: `E[payment] = D`, and `remote_bps` prices the variance. Only cash reduces exposure (D14).
...
- **Solvency**, checked periodically and on `admin identity`: `liabilities` = every positive balance, `vault` = crossings in minus crossings out; `liabilities − vault = 0`, and an alarm names the term that differs. No receivables term: what strangers owe is exposure the operator carries (D14), never backing. Custody compares that vault against the rail's own finalized reads at a settled block — a cut only where the scan covers it and no payment is in flight — the difference bounded by the active refill locks. Both are display, never booking. What the operator risks is its own earnings, up to `credit_limit`, never a claim on a user's balance [→U44, U32].
...
serve NAME serves kernels/<name>/, so two kernels run under one installation root with distinct keys and ports; an empty, dotted, separator-bearing or over-long name is refused, and serve with no name creates nothing
...
a repoint that cannot be reached changes nothing: the recorded address and key stand and every login on that kernel survives; one that answers strands every login on that kernel, not only the context being switched
a context names one kernel and one login; two contexts on one kernel rotate their refresh tokens independently, two processes on one context serialize on the session lock and never spend a rotated-away token, and --context/JUICE_CONTEXT address one without moving the current one
logging smoke test
...
admin deposit/suspend resolve a peer by key (global name) as well as by handle
...
a seller closes an obligation only against a finalized payment from that address, for the amount drawn, carrying the transaction named; one payment closes at most one obligation, and recording the same payment twice moves money once
...
a successful peer gossip pull persists peer_last_seen and the peer's proven rail address; peer sync runs with empty bootstrap_peers; admin peers surfaces what it learned
...
two kernels settle: the debtor pays and announces the fact, the creditor sees the claim and credits it
  against the payment, both books close, a replay moves nothing, a payment with an unknown reference
  closes no debt, and the debtor's calls draw credit again afterwards
three kernels earn and spend: one funds a buyer, the buyer runs the second's action, the two settle,
  and after a restart the second buys from a third out of its earnings; all three books reconcile
