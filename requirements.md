...
| `cancelled` | The process closed before the step was completed. Terminal; no `tx_id`. Not completable.           |
...
`EndProcess` atomically cancels all `waiting` steps tied to the process in the same transaction as closure.
...
waiting steps are cancelled atomically when their process closes; cancelled steps carry no tx_id
