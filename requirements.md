...
- **Reveal.** One signed `{ticket_id, secret, tx_hash?}` on `/juice/fed/settle/1`, bound to a `recipient`: at once for a loss, once the payment is final for a win. The trace remembers the secret and whether the seller has heard, so an unacknowledged reveal is resent. The seller recomputes the draw against the stored commitment, and accepts a loss however late: a deadline turning a loss into a win would make every outage a payment.
