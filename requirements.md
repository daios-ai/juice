...
| `@sys/llm/tools` | Public; action owner `@sys`; price 0 (configurable, `native.llm`, §14); callable through `Call()`. Returns proposed Juice calls only — does not execute them. Input: `messages[]` of `{role,content}`, optional `system`, required `tools[]` of `{action, description, input_schema, output_schema?, price?}`, optional `tool_choice` (`auto`\|`required`\|`none`, default `auto`), optional `max_tool_calls`. Output: `tool_calls[]` of `{action, args}`, optional `message`. Returned actions must be in the supplied tools list; returned args validated against that tool's `input_schema`. `ErrSchemaViolation` for unsupported schemas. `ErrInvalidState` if tool calling unavailable. `ErrExecutionFailed` if `tool_choice=required` and no valid calls produced. |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
Every startup reads `config.superuser_handle` to confirm first boot and identify `@sys`; it verifies signing keys and aborts if either is absent. It then registers, enables, and makes public `@sys/lookup`, `@sys/llm/chat`, `@sys/llm/embed`, `@sys/llm/json`, `@sys/llm/tools`, `@sys/make`, `@sys/time`, `@sys/sink`, `@sys/message`, and `@sys/random` if absent, and reconciles their configurable fields (price and action-specific settings) from config on every startup. It then runs recovery (§5).
...
@sys/llm/tools returns tool_calls with valid action and args
@sys/llm/tools rejects tool call for action not in supplied tools
@sys/llm/tools rejects args that fail tool input_schema
@sys/llm/tools with unconfigured tool calling returns ErrInvalidState
@sys/llm/tools with tool_choice=required and no valid calls returns ErrExecutionFailed
