...
- **U32** Serving strangers is bounded-risk: an operator caps the work this kernel has delivered and not been paid for, across **all** peers at once, and minting additional identities cannot multiply it. Sybil-proof by construction: one bound on the total, never a rule per identity, which a free identity steps around.
- **U33** Every cross-kernel obligation settles on the network's rail, per call. One too small to pay economically pays a fixed larger amount with the probability that makes the expected payment the obligation, so the rail carries one payment per ticket of trade rather than one per call, and neither side can choose the outcome. The seller is paid only against the finalized payment, matched by the fact that names it. A kernel's ledger is backed by cash alone.
...
### P4 Inbound call
- Request signs `JCS({action, args_hash, commitment, counterparty, expected_contract_hash, idempotency_key, lottery, recipient, timestamp})`; `commitment` is SHA-256 of the caller's half of the settlement draw and `lottery` the ticket face value it dispatches under (P10), both omitted when the call owes nothing; a `lottery` above the world's ceiling is refused. The request also carries `rail_address` and `rail_proof` (outside the Ed25519 payload: the proof is the rail key's own signature over the caller's key, D23) — where a winning ticket will be paid from, verified and frozen on the trace at admission, so the seller knows every obligation's payer before any payment can land; on a world with addresses a priced call from a buyer proving none is refused; `action` is the stable id, never a handle-bearing ref (rename-proof); `recipient` is the serving key (a captured request cannot replay to a third kernel); `args_hash = SHA-256(raw body)`. Receiver verifies signature, recipient = own key, body hash, timestamp. A valid unknown key is provisioned an account (handshake-free peering); a suspended key is refused. The serving kernel funds the execution itself (D14). An inbound call naming a cached `remote_proxy` id is refused. [→U29, U37, U47]
...
### P5 Receipt
Fields: `id, issuer_user_id, tx_id, trace_id, action_id, caller_user_id, process_id, args_hash, reply_hash, status, gross, net, fee, charge, premium, nonce, value, value_to, reason, started_at, created_at, signature`. `args_hash`/`reply_hash` = SHA-256 over JCS args/reply. `gross` = full allocation; `fee + net` = amount remaining at settlement; `gross − fee − net` = committed downstream; failure has `fee = net = 0`. `charge` = actually drawn: `= gross` success, `≤ gross` failure (settled descendants stay paid), `0` rejection. `premium = ceil(charge·remote_bps/10000)`. `nonce` is the serving kernel's half of the settlement draw, 32 bytes hex, minted after execution and before the caller's secret is known; a receipt carrying an obligation without one is not settleable (P10). JCS omitempty keeps it out of every free and local receipt. `value ∈ {0, amount}` (all-or-nothing), local and untaxed, so no premium rides it; JCS omitempty keeps pre-transfer receipts verifying. Signature Ed25519 over canonical JSON minus `signature`. Exactly one receipt per committed call; `started_at` call start, `created_at` settlement. [→U36, G1, G7]
...
- success: `D = charge + premium`; `importfee = ceil(D·import_bps/10000)` → origin sys; refund `q − D − importfee` to the caller's wallet, and `D` itself back to the immediate caller C, whose ticket then settles it (P10). No peer row is credited: a peer account holds no money.
...
Every cross-kernel obligation `D` is settled on its own, by a ticket both kernels name by the call's idempotency key scoped to the counterparty. Neither side keeps a record of it beyond the call's own: the trace, its receipt, and the rail row of a winning payment (D4).

- **The draw.** The buyer mints a 32-byte secret before it calls and sends `commitment = SHA-256(secret)` (P4); the seller mints a nonce after executing and signs it into the receipt (P5). Face value `L` is the buyer's, quoted in the request, capped by the world's `lotteryMax`. `D ≥ L` or `L = 0` pays `D` exactly; otherwise pay `L` iff `uint64(SHA-256(ticket_id ‖ secret ‖ nonce)[:8]) mod L < D`, else nothing. `E[payment] = D`. Neither side selects it: the seller cannot know the secret when it draws, the buyer cannot change one whose hash the seller holds. The modulo bias is at most `L/2⁶⁴`, under the whole-unit rounding `D` already carries.
- **The buyer.** It stakes the whole `L` from the immediate caller C's own balance at dispatch whenever the call can owe anything and a lottery is set. Not `L − D_max`: the budget returns `D` to C at commit and the payment is then taken whole. Not zero above `L`: what is drawn on is the actual `D`, which can settle anywhere below the advertised maximum and win. A call whose peer has not proved where it is paid, on a world with addresses, is refused before anything is locked. At settlement the stake is released, and a paying draw reserves the amount from C for the rail in the same commit that decided it.
- **The reveal.** One signed message on `/juice/fed/settle/1` carrying `{ticket_id, secret, tx_hash?}`, bound to a `recipient`. A losing draw is revealed at once; a paying one waits until its payment is final, then names it; the sender was proven with the call (P4). The buyer's records are the durable memory — the trace holds the secret and whether the seller has heard — so an unacknowledged reveal is sent again. The seller recomputes the draw against the commitment it stored, so a buyer cannot report a loss on a ticket that won; a late cancel still closes the obligation, since a deadline turning a loss into a win would make every outage a payment.
- **The seller.** It closes an obligation only against a finalized payment from that proven address, for the amount drawn, carrying the transaction named; one payment closes at most one. A buyer that never reveals is bounded by the credit limit like any other, since only cash reduces exposure.
- **Ledger identity.** A call moves nothing between kernels by itself: the budget pays `D + importfee`, `D` returns to C, and C's stake carries the draw. Cash crosses only when a ticket pays: the buyer books `C −L` into the operator's hold and out; the seller books `L` in and credits all of it to the provider. Neither operator keeps any part of a draw — `E[payment] = D`, so retaining `L − D` would underpay every seller that wins — and `remote_bps` prices the variance the traders carry. Only cash reduces exposure (D14).
...
- Preconditions, exact order: (1) `C` authenticated, not suspended; (2) process open; (3) supplied parent trace exists in the process; (4) `C` may use the process — `C = P`, or parent trace's action owner = `C`, or step completion by required caller; (5) action exists; (6) `CanCall(C, action)`; (7) quote pin matches; (8) args satisfy input schema; (9) trace `available ≥ price` (step completion funded by its parked price instead), and `C` covers its own stakes — the transfer value it delivers and the lottery ticket a cross-kernel call stakes (P10). Precondition 4 governs spending authority, 6 governs access — distinct questions; the 6-before-7 order is why a pin mismatch never leaks a private action's terms [→U48] and terms-invalidating-args still reports `ErrTermsChanged`.
...
- **Trace**: `id, process_id, parent_trace_id, action_owner_id, available, locked, idempotency_key, dispatch_json, ticket, revealed, owed_status ∈ {∅, announced, cancelled, credited}, owed_amount, owed_tx_hash, owed_rail_address, value, value_to, idempotency_record_id, created_at`. The call's wallet, and both sides' whole memory of its draw: an obligation is never a row of its own. Buying, `ticket` snapshots the lottery stake locked from the immediate caller and `revealed` that the seller has heard how the draw came out. Selling, the four `owed_*` fields carry the buyer's reveal — what moves, the payment named, the proven sender it comes from — while what is owed is the receipt's `charge + premium`, the terms are the frozen record, and the peer's name for the call is its idempotency record; `owed_rail_address` is the payer proven with the call and frozen at admission (P4), so a later change cannot retarget a payment already waited for; `value`/`value_to` snapshot a transfer effect (D18) — the amount locked from the caller and its beneficiary, riding the trace so every settlement path releases the lock from the snapshot and rates are pinned against config changes [→G4]; all 0 on a non-transfer call. `idempotency_key/dispatch_json` null except on a call that crosses a kernel boundary, in either direction: what the buyer dispatched under, or what the seller admitted under (D19); `idempotency_record_id` = the inbound record this call answers, set for any kind, so whichever settlement resolves the trace completes the record. `process_id` denormalized. Latency never cached (D22).
...
- **Rows**: one row per external movement. A `deposit` is an inbound fact, always booked `held`: who it belongs to is decided by one reconciliation, which closes the obligations whose money it is before attributing the rest to the account that registered the sending address. A payment from an address some admitted foreign call named as its payer stays held while that call's obligation is unresolved — unrevealed, or revealed and unpaid: a buyer's winning payment can land before its reveal, from an address a local account also registered, and crediting that account on sight would leave the seller unpaid for good. The payer is frozen with the call, so this holds for a buyer this kernel had never heard of. No deadline releases it, since a late losing reveal is valid; a loss frees it on the next pass. A `payout` — a user's withdrawal, or a won ticket on its way to a peer — runs `pending → submitted | refilling | blocked → confirmed | failed`, snapshotting its destination when it reserves so nothing redirects it, and a `blocked` row is re-presented in place, changing status only on success. A payment the rail signed and then refused is re-presented too, since the debt did not go away, but under a fresh `attempt`: a settled rail operation cannot be asked again, so reusing its name would ask nothing at all forever. A confirmed row is done, since telling the seller which payment settles its obligation is the ticket's business (P10). A `refill` is the fuel lock. Money in transit is held on `sys`, never spendable (D3). The world file also carries `lotteryMax`, the largest ticket a kernel here may write: what a payment costs is a property of the rail, so the ceiling rides with it, outside the defining part.
- **Halt**: derived, never stored — while any row is `blocked`, outgoing rail work refuses `ErrRailStopped` while deposits, paid work, and reads continue, and the local fees that accrue are what clear it. Money verbs also refuse until the full domain check — chain, token, decimals, venue — has passed, and the worker does nothing at all before then: an unverified token misstates every amount.
- **Refill**: fuel is `sys`'s cost alone, carried by prices and `fee_bps` and never by user backing. The kernel passes what is not `sys`'s to spend as the rail's reserve, and the rail's one rule refuses, before signing, any refill that would dip into it. Before the rail is asked, everything `sys` could spend is locked, so the reserve holds until the purchase is known; the lock then settles at the purchase's authorized maximum and is released at booking against the exact amount consumed: the maximum is authority, never cost. A purchase is durable before it is broadcast and the rail carries one at a time, so one that never got out is presented again rather than repeated, and one the ledger never bound is adopted from what the rail holds.
- **Solvency**, checked periodically and on `admin identity`: `liabilities` = every positive balance, `vault` = crossings in minus crossings out; `liabilities − vault = 0`, and an alarm names the term that differs. No receivables term: the ledger is cash-backed, and what strangers owe is exposure the operator carries (D14), never backing. Custody compares that vault against the rail's own finalized reads at a settled block — a cut only where the scan covers that block and no payment is in flight — the difference bounded by the active refill locks. Both are display, never booking. Credits are backed by the vault alone. What the operator risks is its own earnings: work delivered on credit up to `credit_limit` that may never be paid for, and never a claim on a user's balance.
- **Addresses**: money is credited by the finalized sender, never by whoever reports the payment, so money from an exchange must hop through the user's own wallet first. A user registers an address by signing the kernel's registration message with it; that attributes the sender's held deposits retroactively, and one address serves one account. Replacement needs only a new signature: an in-flight withdrawal carries its own destination. A kernel serves its own rail address with a signature by its rail key: a merely declared address could name a third party's and claim their payment as this kernel's settlement. Senders nobody has registered stay `sys`-held and listed until the operator assigns one.
...
- Migration `049` refuses to apply while any of the bilateral economy's value is still in flight: a peer row holding anything, a call whose funds the dropped serving-markup column backs, or a settlement drawn for and unfinished. Once it applies, every dispatched call carries a full pricing record, so settlement reads one and the fallbacks that guessed at missing fields are gone.
- The pre-snapshot receipt fields are kept rather than renamed: `premium` still names the serving markup, now owed by the buyer rather than credited to a row, so receipts written under the bilateral economy verify unchanged. `nonce` is added under omitempty, so every one of them still verifies.
