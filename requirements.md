...
- **U33** Kernels settle obligations over the network's rail: the kernel meters who owes whom, pays, and closes the debt only against the finalized payment, matched by the fact that names it. Debts too small to pay economically still settle fairly, with zero expected loss to either side.
...
- **U44** The operator can observe everything material — parked remote calls with age, peers with balances, evidence about any subject, and unsettled bilateral obligations, and the rail position: holdings, money in transit, solvency, and any halt with its cause — and intervenes with the same commands users have, widened in scope, plus the money-and-trust verbs only an operator performs.
...
- signed rejection: full refund; a non-402 rejection deactivates the proxy iff the row still holds the dispatched hash (a stale dispatch spares a refreshed row). A rejection is identified by `tx_id` = the dispatched `idempotency_key` (P4) — never by charge or transport status, both of which an executed failure shares — and classifies: 402 → `ErrPeerUnfunded` (operator condition); `refresh_proxy` → ordinary failure with `Meta["retry"]="refresh"`, never `ErrTermsChanged`; otherwise → `ErrUnauthorized`, the peer refuses us. An executed failure stays `ErrExecutionFailed` at any charge and keeps the row.
...
- Exact (`d ≥ Q` or `Q = 0`): the debtor reserves `d`, pays it on the rail to the creditor's proven address, and announces `settlement_id → tx` on the settle stream; the creditor closes the debt against the finalized payment that transaction carried from the debtor's proven address, for the amount claimed; one payment closes at most one claim, so two settlements of equal amount cannot be confused. An announcement naming no finalized payment settles nothing.
- Probabilistic (`0 < d < Q`): debtor-driven commit/reveal. Creditor is stateless: `s = SHA-256(signing_seed ‖ "juice-settle-v1" ‖ settlement_id)`; the debtor carries the creditor-signed *open* record to *finish*. Outcome: pay iff `SHA-256(settlement_id ‖ s ‖ n) mod Q < d` (`n` = debtor nonce; modulo bias negligible for `Q ≪ 2⁶⁴`). The lottery moves no money; the debt stays on both rows until the cash `Q` moves through the same reserve-pay-announce path. `E[payment] = d`. One debt draws once: while a payable draw stands unpaid the creditor opens and finishes nothing else for that peer, and a clear applies only while the debt it forgives is still outstanding — otherwise a debtor would redraw under fresh ids until a clear came up and pay nothing. The debtor writes the draw down before its first round — its id and the nonce it will reveal, then each round's signed answer — and resumes it by id, so a lost reply or a crash finishes the same draw rather than starting another the creditor never agreed to. [→U33]
- `SettlementRecord` binds both keys, `d`, `Q`, mode (`exact` or `probabilistic`), the destination, commitment `H(s)`, nonce, revealed `s`, outcome, expiry, `settlement_id`; its key-set is disjoint from every other signed payload. Exactly one outcome per `settlement_id` per kernel, ever; a replayed finish (including a ground nonce) is re-served the stored record, never a second signature. Creditor non-reveal past expiry ⇒ debtor applies clear-for-zero with the signed open record as evidence and re-presents it via `reconcile`; the creditor must apply the same (idempotent) or be in provable default. Debtor refusal leaves the debt outstanding consistently, consuming exposure.
- Ledger identity per kernel: `Δrow + Δsys = external_cash`. finish→clear: creditor `row +d / sys −d`, debtor `row −d / sys +d`, cash 0 (immediate, internal). finish→pay: no balance change (records the pending obligation). Paying cash `Q` (exact: `Q = d`): the debtor's reserve and finalization together book `row −d, sys −(Q−d)`, cash out `Q`; the creditor books `row +d, sys +(Q−d)`, cash in `Q`, when it credits the announced payment. Any leg debiting `sys` past reserve aborts whole, `ErrInsufficientFunds`; a finalized payment failure compensates the reserving commit exactly, leaving the debt standing. Residual variance is operator variance on `sys`: the creditor gains `Q−d` on pay and loses `d` on clear, the debtor the mirror, so `E[Δsys] = (d/Q)(Q−d) + (1−d/Q)(−d) = 0`; kernels hold reserves against it.
...
- **Kernel**: `public_key, petname, nickname, about, rail_address, rail_proof, gossip_cursor, last_seen, last_contact_failed_at, peer_credit, first_seen, updated_at`. Owns naming state (D15); cursor advanced only after verified commit (P9); the two contact timestamps and `peer_credit` are display cache only (D16); location never stored; discovery creates no account.
...
- **One loop** (`discovery_interval_seconds`): each pass enumerates namespace providers and pulls gossip from providers ∪ bootstrap seeds ∪ counterparties; a verified pull refreshes the `Kernel` row (nickname, about — never petname) and evidence cursor, and persists `peer_credit = counterparty_balance` when the peer reported one.
...
- **Rows**: one row per external movement. A `deposit` is an inbound fact, `held` until its sender is known and then `credited`; a `settlement` being drawn for is `drawing` first — the row is the draw's durable memory and moves nothing until the draw says pay, when it becomes the payment in place, or clear, when it goes — and a `payout` or `settlement` runs `pending → submitted | refilling | blocked → confirmed | failed`, snapshotting its destination when it reserves so nothing redirects it, and a `blocked` row is re-presented in place, changing status only on success; a `claim` is a peer's announced settlement, closed by the deposit that matches its fact; a `refill` is the fuel lock. Money in transit is held on `sys`, never spendable (D3).
...
- **Solvency**, checked periodically and on `admin identity`: `liabilities` = every positive balance, `receivables` = every peer shortfall (bounded by `X`, D14), `vault` = crossings in minus crossings out; `liabilities − receivables − vault = 0`, and an alarm names the term that differs. Custody compares that vault against the rail's own finalized reads at a settled block — a cut only where the scan covers that block and no payment is in flight — the difference bounded by the active refill locks. Both are display, never booking. Credits are backed by the vault plus peer IOUs up to `X`: in the worst case users jointly hold `X` more credits than the vault can pay, the operator's accepted, priced risk.
...
a debt drawn and lost is not drawn again until it is paid, and one debt is never cleared twice; a draw whose answer was lost is finished by its own id, never redrawn, and both ledgers agree on its outcome
a paid outcome stays open until the cash Q moves: the debtor reserves, pays, and announces the fact, booking row −d and sys −(Q−d) with cash out Q; the creditor credits the announced payment only when transaction, sender, and amount match, booking row +d and sys +(Q−d) with cash in Q; an announcement naming no finalized payment settles nothing and an unannounced payment closes no debt; two settlements of equal amount resolve by their own facts; a sys leg short of reserve (either side) refuses insufficient_funds, no partial writes; replay is a no-op; a payment failure compensates the reserving commit exactly and the debt stands; a pending pay blocks the debtor's further credit-drawing calls
every settlement record is conservative (Δrow+Δsys=0) and the cash Q crosses on its own rail row, never in the record; E[variance]=0
...
settle payload domains (settle_open, settle_finish, settle_announce, settle_reconcile) are disjoint from each other and from step_auth; a payload signed for one network never verifies on another
