...
| Import OpenAPI | `POST /v1/actions/import` `{spec_url[, as]}` → import result | `juice action import <spec-url> [--as <prefix>]` |
...
| Deposit credits | `POST /control/deposit` `{handle, amount, [reason], [external_key]}` | `juice admin deposit <user\|key> <amount> [--reason --external-key]` |
| Withdraw credits | `POST /control/withdraw` `{handle, amount, [reason], [external_key]}` | `juice admin withdraw <user\|key> <amount> [--reason --external-key]` |
