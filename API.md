...
Federation has no HTTP surface: peer identity, gossip, manifests, and inbound calls travel over the cross-kernel transport (D12), not over this API. Kernels discover each other in the background through libp2p routing discovery over a fixed namespace (D12); discovered actions surface through `sys/lookup`, and every known kernel — counterparties and discovery-only alike — appears in the merged `admin peers` roster and is inspected with `admin inspect <key>`.
...
| Settle a peer | `POST /control/peers/settle` | `juice admin settle <peer> [--cash <settlement_id>]` — settles the bilateral position: exact if debt ≥ `Q`, else the probabilistic residual protocol (P10); `--cash` records the rail payment for a paid outcome |
