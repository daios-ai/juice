...
**R3 — Action identity is `owner@kernel/name`.**  
Wherever a request identifies a callable action, a single `action` field carries its address, `owner@kernel/name`. The server resolves it. Split fields (`target` + `action_name`) are forbidden. Handles must not contain `/`; action names may (`bob@k/mail/inbox`), which is what makes directory selectors possible (D10). A reference resolves the exact action it names, else that path's `index` child — the web's index-page convention — at every depth including zero: `bob@k` reaches `bob@k/index`, `acme@k/mail` reaches `acme@k/mail/index`. `index` is an ordinary action; there is no application object. Parsing is unambiguous: the first `/` ends `owner@kernel`, and the name is everything after it. In a URL query string, `/` must be percent-encoded.
...
No envelope objects. `GET /v1/steps` returns `[…]` directly. Two replies are composite views rather than one collection and are named as such: `GET /v1/admin/kernel/deposits` answers `{deposits, owed}` — two real lists, since flattening them would invent a third thing — and `GET /v1/steps?peer=` answers `{steps, truncated}`, a peer's page plus whether more are waiting. Metadata such as pagination belongs in response headers, not the body. Every list endpoint pages by `limit`/`offset` query params — default `limit` 50, ceiling 200, `offset` floored at 0 — so no single response is unbounded.
...
A user is an address, `handle@kernel`; an action is `owner@kernel/name` (a raw id is also accepted). The kernel segment is a kernel's name as this kernel knows it — its own name for its own users and actions, a peer's local name or public key for a peer's — and a bare `handle` or `owner/name` is refused everywhere (D15). Processes, steps, and transactions, which have no human-readable name, are ids. Outputs render every party as an address, never a raw id; a peer kernel as a party is its name alone.
...
`juice step list` lists steps. Optional filters (`--process`, `--status`) narrow the result, and `--limit`/`--offset` page it, without changing the command group. `juice step list --peer <key|petname>` (`GET /v1/steps?peer=`) instead asks that peer for the steps it holds for the caller — an ordinary user sees the steps addressed to them, the superuser every step this kernel may complete — as `{steps, truncated}`: one bounded page in the peer's order, with `truncated` set when more are waiting. It takes no filter or paging parameter; combining one with `peer` is `ErrInvalidInput`.

**C7 — What is known about an action is read where the action is read.**  
`juice action show <action>` carries the `evidence` block; there is no separate stats surface, so no reader can see a different record of the same action than another.
...
The step completer is identified at creation time by address, `user@kernel`, here or on a peer. The server resolves it to `required_caller_user_id`, plus `required_caller_remote_id` for a remote principal, whose completion is signed by the home kernel with that user's id inside it (P8). Completion is rejected if the authenticated caller does not match.
...
Every command that addresses a kernel calls it over HTTP; `kernel list`, `kernel forget` and `auth list` read and write this client's own records and call nothing. There is no `--db` and no `--config`: a kernel *is* its `$JUICE_HOME/kernels/<world>/`, which only `kernel serve` reads (`kernel serve <world>`, the network it serves, defined by the file of that name in `$JUICE_HOME/worlds/`); `config.json`'s `listen_addr` binds the HTTP API and its `fed_listen_addrs` the peer transport (empty ⇒ the standard port 31313, refused if held), and every key of that file is also a flag of `kernel serve` — the key with `_` written `-`, a nested key as a path — which outranks the file for one run. Under `$JUICE_HOME/client/` the CLI keeps the kernels it knows — endpoint, public key, network fingerprint — and one 0600 file per login, named `handle@kernel`, holding its tokens. Commands are `juice [admin] <noun> <verb>`, with `run` the one exception. `juice kernel add <url>` registers a kernel after reading `GET /health`, under the name it calls itself, selecting nothing; `juice auth login <user>@<kernel>` authenticates and acts as that login, `juice auth use` switches between logins held, and both refuse a server whose public key or network fingerprint is not the recorded one. `--as <user>@<kernel>` or `JUICE_AS` names one for a single invocation and refuses a name that is not a login here. `kernel list`, `kernel forget` and `auth list` appear in no table below and call nothing: they read and write this client's own records, and a kernel has no say in what its callers choose to remember about it. Every other command is a request to a server, and every one of those is a row below. `--server <url>` overrides one invocation's endpoint, and credentials travel only to the address recorded for that login's kernel — never to an overridden one, since what a server says about itself is a claim rather than a proof. `admin *` are superuser supervision served on that same public TCP API, on routes gated by an `IsSuperuser` check — authority is the `sys` bearer token (keep it secret; expose `kernel serve` only behind TLS or on loopback), not a separate socket or filesystem access. `juice kernel serve` is the sole process that opens the database. Supervision over ordinary resources is **scope**, not a separate surface: a superuser sees all rows on `action/process/tx/step list` and may `action disable`/`enable` any action, all over the normal TCP API.
...
Money is one unit throughout the CLI, the world's: `--price`, `admin user deposit`, `user transfer` and `user withdraw` all take an amount written the way the same kernel shows it (`1.5`, not `1500000`), and all refuse more decimal places than the world has. JSON bodies and every number a read returns are base units. The world's decimals come from `GET /health`.
...
| Operation | HTTP | CLI |
|-----------|------|-----|
| Authorize (PKCE) | `POST /v1/auth/authorize` `{handle, password, code_challenge, [redirect_uri]}` → `302` if `redirect_uri` provided, else `200 {"redirect":"?code=CODE"}` | `juice auth login <user>@<kernel> [--password]` |
...
| Recover (complete) | `POST /v1/auth/recover/complete` `{handle, nonce, signature, password}` → `{status}` | `juice auth recover <user>@<kernel> [--phrase] [--password]` |
...
| Operation | HTTP | CLI |
|-----------|------|-----|
| Create user | `POST /v1/users` `{handle, password, [recovery_public_key]}` → 201 user | `juice user create <user>@<kernel> [--password]` |
| Get self | `GET /v1/me` → user (`id`, `address`, `description`, `available`, `locked`, `blockchain_address`, `connectors` the directory-grouped tree each `{directory, connections, actions:[{action, scopes, provider_key, created_at}]}`, `connections` the account inventory each `{provider, actions, unused, provider_key, created_at}`) | `juice user me` |
...
| Disconnect grants | `DELETE /v1/grants?selector=` → `{revoked: [actions]}` (a full `owner@kernel/name` is the degenerate one-action selector) | `juice user disconnect <selector>` |
...
| Operation | HTTP | CLI |
|-----------|------|-----|
| Run action | `POST /v1/run` `{action, args, [quote_hash]}` → `{result, tx_id, trace_id, receipt_id, process_id, charge}` | `juice run <action> [json] [--quote-hash H]` |

`charge` is what the call drew from the caller, in base units: the price on success, only what sub-work already delivered consumed on a failure, zero for a free action. It is absent exactly when `tx_id` and `receipt_id` are — a call whose settlement is deferred has drawn nothing yet. An executed failure answers with its error rather than this body, and carries `tx_id` and `charge` in the error's `meta`.
...
| Verify receipt | `GET /v1/transactions/{id}/receipt-verification` → verification | `juice tx verify <id>` |

A caller reads transactions where it is a captured party: payer, caller, or payee; a superuser reads all. Responses render each party as an address — `owner` (payer), `caller` (caller), `target` (payee) — and the action as `action` (`owner@kernel/name`); the raw `*_user_id` UUIDs and the stored `action_name` are not returned, since a user is addressed by address, never an id (a purged party falls back to its raw id). `rating` must be 0 (bad) or 1 (good); `note` is an optional string. Transaction and list responses include a `rating` field — `{"value": 0|1, "note": string|null}` when rated, `null` when unrated. Transaction `args` and `result` fields are inline JSON objects. `gross` is the call's full allocation; `fee + net` is the value added paid out at settlement. A cross-kernel transaction also carries `ticket_id`, the name both kernels know the obligation by, so either party can follow it and an operator recording its payment can name it. The obligation itself is a row on the side that is owed; the buyer's side of it is the receipt on this transaction and the payment on the rail (P10).

Every transaction has a signed receipt and `receipt-verification` audits it entirely from local data. `checks` is a map naming each audit that applied and whether it held; an audit that does not apply to the transaction is absent, never reported as passed, and `valid` is true iff every check that ran held. A local transaction's receipt is verified against this kernel's own key: signature, `action_id`, `status`, `charge`, the signed `gross/net/fee` against the row, and the args/result hashes. A remote-proxy transaction (which also includes `remote_receipt_hash` and `remote_receipt_json`) is verified in two parts: **receipt integrity** (signature against the peer's public key, stored JSON against its stored hash, `action_id` against the proxy) and **settlement consistency** (local outcome matches `receipt.status`; the obligation equals `receipt.charge + receipt.premium`; the premium and import fee at the rates the dispatch froze; the refund arithmetic; the ticket paid what the recorded secret, the receipt's nonce and the obligation decide). A remote receipt's own `gross/net/fee` are the remote kernel's economics and are reported, not compared.
...
| List steps | `GET /v1/steps[?process_id=&status=&limit=&offset=]` → step[]; each carries `created_by` (the creating action's address, from the parent trace) alongside `action` (the completion target), `owner` (the payer) and `required_caller` (the completer's address; a peer kernel's name alone when the step is addressed to the kernel itself) | `juice step list [--process --status --limit --offset]` |
...
| Operation | HTTP | CLI |
|-----------|------|-----|
| List all users | `GET /v1/admin/users[?limit=&offset=]` | `juice admin user list [--limit --offset]` |
| Show an account | `GET /v1/admin/users/{address}`, `GET /v1/admin/peers/{key\|petname}` | `juice admin user show <user@kernel>`, `juice admin peer show <key\|petname>` — a kernel target returns one flat record: `petname`, `nickname`, `public_key`, `about`, the sync cache, and the account state when one exists |
| Suspend | `POST /v1/admin/users/{handle}/suspend`, `POST /v1/admin/peers/{key\|petname}/suspend` | `juice admin user suspend <user>`, `juice admin peer suspend <key\|petname>` — freezes any account: a human cannot log in; a peer's inbound calls are refused with a signed rejection |
| Unsuspend | `POST /v1/admin/users/{handle}/unsuspend`, `POST /v1/admin/peers/{key\|petname}/unsuspend` | `juice admin user unsuspend <user>`, `juice admin peer unsuspend <key\|petname>` |
| Rename | `POST /v1/admin/users/{address}/rename`, `POST /v1/admin/peers/{key\|petname}/rename` | `juice admin user rename <user@kernel> <new@kernel>`, `juice admin peer rename <key> <petname>` — a local account target renames its handle (freeing the old one); a kernel target (petname or key) binds its **petname**, exactly, rejecting an occupied name. This is the only way to name a kernel this node has merely discovered |
| Record money in | `POST /v1/admin/users/{handle}/deposit` `{ref, [amount], [reason]}` | `juice admin user deposit <user> [<amount>] --ref <fact> [--reason]` — two forms: record a payment made outside the system (`<user> <amount> --ref <fact>`; a second use of the same fact moves nothing, the same fact with a different amount is 422, no `--ref` is 422); attribute a payment already received whose sender was unknown (`<user> --ref <txhash>[:<index>]`). Only a user is ever credited: a handle, id, or nothing — a peer is not a user here, and a refusal provisions nothing. 404 for a fact nobody has seen, 409 for one already attributed |
| List what this kernel is waiting on | `GET /v1/admin/kernel/deposits` → `{deposits, owed}`: payments held for a sender nobody has registered (every observed payment is booked held and reconciled — obligations first, then the account that registered the sending address), and every obligation still open — the work delivered to foreign buyers and not paid for, which is what `exposure` is made of, itemised. Each carries its `peer`, its `obligation`, and once its buyer has said how the draw came out, the `status`, the `amount` drawn and the `tx_hash` named; a buyer that has said nothing yet is listed with an empty `status`. A payment from an address an admitted foreign call named as its payer is never attributed to a local account while that obligation is unresolved, so a winning payment that lands before its reveal waits for it. A held payment's `id` is what `--ref` names to attribute it, by that id or by the bare `<txhash>[:<index>]` it is built from; an obligation needs no such act, since it closes when its buyer pays | `juice admin kernel deposits` (bare) |
| List peers | `GET /v1/admin/peers[?all=&limit=&offset=]` | `juice admin peer list [--all --limit --offset]` — every known kernel from one query, this kernel excluded: counterparties (`has_account=true` and the D16 contact cache `last_seen`/`last_contact_failed_at`) and discovery-only kernels (`has_account=false`, `actions` count). `petname` is the local name that **resolves** a reference; `nickname` is what the kernel calls itself and never resolves (D15) — an unbound kernel shows `—` and stays callable by key. No internal id; `--all` also lists suspended counterparties |
| Inspect a kernel | `GET /v1/admin/peers/{key\|petname}/inspect` | `juice admin peer inspect <key\|petname>` — identity (incl. its `about`), public actions (with descriptions), retained evidence grouped by issuer (trade-backed vs unverified), reachability. `source` is `live`/`local`/`none`: an offline but known peer degrades to last-known local data (`online:false`) |
| Show own identity | `GET /v1/admin/kernel` | `juice admin kernel show` → `{handle, public_key, about, addrs, network, network_fingerprint, blockchain_address, finalized{token, gas, block}, sys{earnings, pending_payouts, held_deposits, refill_locks}, solvency{liabilities, vault, gap}, custody{checked, difference, ok}, stop{reason, since}, fee_bps, remote_bps, import_bps, lottery, lottery_max, credit_limit, exposure}` — identity, the rail position (D23), the money rules this kernel serves under, and its credit position (D14): what it has delivered unpaid, against the limit it set |

The route says which sort of target it takes — `users` or `peers` — and a target of the other sort is refused, so a petname that happens to equal a handle never decides which account is acted on; a user target is an address on this kernel, a peer target a petname or key, never this kernel's own name. The deposit sits under `users` for the same reason: it credits a user, so it resolves handles alone and a petname that happens to equal one is simply not a user. A target is the full identifier set of its noun: under `users` a handle or an account id, under `peers` a petname or a public key (D15). Every roster verb answers with the target as `show` renders it, so what a change did needs no second request to read (R5). `<user>` is a handle or an account id, always a user of this kernel; `<key>` is a peer's petname or its base64url public key, always a kernel; `<action>` is `owner/name` (or an id). A deposit never names a peer: a peer account is identity, not a wallet, and what a peer owes closes when it pays — the chain payment observed where the world has a chain, and the buyer's own signed reveal where it has none, recorded by the seller's kernel itself (peering is handshake-free, established by a peer's first call); the `about` shown by `identity`/`inspect` is `sys`'s user description, set with `juice user update --description`. Blocking a peer's inbound calls is `admin peer suspend <key>` (reversible with `admin peer unsuspend`); a caller that no longer wants a peer's actions simply stops calling them, and the cached proxies lapse through peer retention (D16). A peer account holds no session token, so a step whose required caller is a peer is completed by this kernel's operator with `juice step complete <id> --peer <key>` (superuser scope on the ordinary command); `admin peer inspect <key>` lists what a peer holds for us. Both are refused for a suspended peer.