...
Every command runs by calling the server over HTTP. There is no `--db` and no `--config`: a kernel *is* its `$JUICE_HOME/kernels/<name>/`, which only `serve` reads (`serve <name>`, the kernel's nickname); `--addr` binds the HTTP API, and `config.json`'s `fed_listen_addrs` binds the peer transport (empty ⇒ OS-assigned ports). Under `$JUICE_HOME/client/` the CLI keeps the kernels it knows — endpoint, public key, network digest — and the contexts naming one kernel and one login on it, each context's tokens in its own 0600 file. `juice use [<name>] [--endpoint <url>] [--kernel <name>]` switches context and verifies it against `GET /health`, refusing a server whose public key or network digest is not the recorded one; bare, it lists the contexts. `--context <name>` or `JUICE_CONTEXT` addresses one for a single invocation. `--server <url>` overrides one invocation's endpoint (default `http://localhost:4040`), and credentials travel only to the context's own recorded address — never to an overridden one, since what a server says about itself is a claim rather than a proof. `admin *` are superuser supervision served on that same public TCP API, on routes gated by an `IsSuperuser` check — authority is the `sys` bearer token (keep it secret; expose `serve` only behind TLS or on loopback), not a separate socket or filesystem access. `juice serve` is the sole process that opens the database. Supervision over ordinary resources is **scope**, not a separate surface: a superuser sees all rows on `action/process/tx/step list` and may `action disable`/`enable` any action, all over the normal TCP API.
...
| Inspect a kernel | `GET /control/peers/inspect?key=` | `juice admin inspect <key\|petname>` — identity (incl. its `about`), public actions (with descriptions), retained evidence grouped by issuer (trade-backed vs unverified), reachability. `source` is `live`/`local`/`none`: an offline but known peer degrades to last-known local data (`online:false`) |
