...
| Operation | HTTP | CLI |
|-----------|------|-----|
| Health check | `GET /health` (open) → `{status, handle, public_key, network, network_digest, decimals, symbol, token, rail_address, fed_addrs}`; identity banner — what a client pins before it trusts a server (C13); `token` is the contract money is paid in, empty where the world has no chain; `fed_addrs` are the addresses peers dial this kernel on, empty until its transport is up | `juice kernel health [<name>]` |
