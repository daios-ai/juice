...
Subscription is optional: a call to a remote action `owner@kernel/name` resolves and caches it on demand (§13), and `admin subscribe` just bulk-warms the whole catalog. Either way a peer action is addressed as `owner@kernel/name` (the canonical principal form; the legacy `kernel/owner/name` mount form also resolves) and called through `POST /v1/run` like any local action; there are no federation HTTP endpoints. Imported proxies are enabled with `visibility = local`, so a kernel exposes only its **own** `public` actions to peers — a peer's own imports are never re-advertised *and* an inbound peer call to one is denied by `CanCall` (a peer caller fails the `local` branch, §4), so subscription is non-transitive at both the manifest and the call layer (reach a peer's imported action by subscribing to its true owner). Trust and peering are managed via the admin commands below. The cross-kernel transport is an implementation detail (§13).
...
| Set peer credit | `juice admin credit <peer> --max <amount> [--settle-at <amount>]` — the bilateral credit this kernel extends the peer (§13): its balance may go negative down to `−max`; `0` (default) is prepaid-only |
