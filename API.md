...
| Deposit credits | `POST /control/deposit` `{handle, ref, [amount], [reason]}` | `juice admin deposit <target> [<amount>] --ref <fact> [--reason]` — three forms: record a payment made outside the system (`<user> <amount> --ref <fact>`; a second use of the same fact moves nothing, the same fact with a different amount is 422, no `--ref` is 422); attribute a payment already received whose sender was unknown (`<user> --ref <txhash>[:<index>]`); record a peer's settlement payment (`<peer> [<amount>] --ref <settlement id>`). 404 for a fact nobody has seen, 409 for one already attributed |
| List money awaiting attribution | `GET /control/deposits` → payments held for a sender nobody has registered, and peers' announced settlement claims, each with its age | `juice admin deposit` (bare) |
| Settle a peer | `POST /control/peers/settle` `{handle}` → `{settlement_id, mode, status, amount, tx_hash, handle, [outcome]}` | `juice admin settle <peer>` — settles the bilateral position: exact when the debt reaches `settlement_quantum`, else the probabilistic residual protocol (P10). This kernel pays on the rail and announces the fact; the creditor closes the debt by recording that payment with `admin deposit <peer> --ref <settlement id>` |
| List peers | `GET /control/peers[?all=&limit=&offset=]` | `juice admin peers [--all --limit --offset]` — every known kernel from one query, this kernel excluded: counterparties (`has_account=true`, `available`/`locked`, the D16 contact cache `peer_credit`/`last_seen`/`last_contact_failed_at`, `settlement_due`) and discovery-only kernels (`has_account=false`, `actions` count). `petname` is the local name that **resolves** a reference; `nickname` is what the kernel calls itself and never resolves (D15) — an unbound kernel shows `—` and stays callable by key. No internal id; `--all` also lists suspended counterparties |
