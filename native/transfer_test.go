...
func TestTransferLocal(t *testing.T) {
	...
	reply, err := k.Run(ctx, kernel.RunRequest{CallerID: alice.ID, ActionRef: "sys/transfer", Args: map[string]any{"target": "bob", "amount": float64(100)}, QuoteHash: pinFor(t, k, "sys/transfer")})
	if err != nil {
	...
	if _, err := k.Run(ctx, kernel.RunRequest{CallerID: alice.ID, ActionRef: "sys/transfer", Args: map[string]any{"target": "bob@other", "amount": float64(10)}, QuoteHash: pinFor(t, k, "sys/transfer")}); !errors.Is(err, kernel.ErrInvalidInput) {
	...
	if _, err := k.Run(ctx, kernel.RunRequest{CallerID: bob.ID, ActionRef: "sys/transfer", Args: map[string]any{"target": "alice", "amount": float64(200)}, QuoteHash: pinFor(t, k, "sys/transfer")}); err == nil {
