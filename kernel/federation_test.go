...
func TestImportRemoteActionReimp(t *testing.T) {
	...
	m.Price = 99
	sig2, err := testNet.SignManifest(priv, &m)
	if err != nil {
		t.Fatal(err)
	}
	m.Signature = sig2
...
func TestPurgeIdlePeers(t *testing.T) {
	...
	kDisabled := newKernel(testConfig(), kernel.Dependencies{Store: st})
	...
	kEnabled := newKernel(cfg, kernel.Dependencies{Store: st})
...
func TestNetworkAmountReadsAsMoney(t *testing.T) {
	token := kernel.Network{Decimals: 6, Symbol: "USDT"}
	...
	for _, tc := range []struct {
		...
		{token, 1500000, "1.50 USDT"},
		{token, 1000001, "1.000001 USDT"},
		{token, 1500, "0.0015 USDT"},
		{token, 0, "0.00 USDT"},
		{token, -1500000, "-1.50 USDT"},
